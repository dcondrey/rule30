...
The kernel `(1+z+z^2)^d` has support `3^popcount(d)`, so survival depends on the
...
`d = n-1-t`. The kernel `(1+z+z^2)^d` has support `3^popcount(d)` for
low-popcount `d` and density 2/3 for `d = 2^m - 1`. The variation is a property
