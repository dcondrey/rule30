...
6. **R1's forcing hypothesis is answered NO** by prior logs, re-read this
   session: a periodic `c` does not force `r|Z` periodic in the driven RHP
...
9. **P1 correction landed** in `RESULTS-DISTINCT-CONTINUATION-COUNT.md`: full
   ...
   `n=21` `k_dep=8`).
10. **Task list written** (`TASKLIST-20260904-R1.md`), R1-first, with every
...
| — | `H(p,w)` growth in `w` | **UNMEASURED beyond `w=8`.** This is A7 and it is the one that decides whether R1 is reachable by compute at all |
