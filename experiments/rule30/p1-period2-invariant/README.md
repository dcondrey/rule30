...
| Dual colex, phase gaps, and pull ancestry | Both inherited scans strictly colex-descend; the phase-gap cover is proved for all lengths. Every nonfirst retreat has one productive pull, and every post-retreat pull is exactly `102->0021` on `p-t=N-3`. The appended-coordinate parent forest obeys `h_i=h_(i-1)+1` for pull gaps at least three and `h_i=h_(i-2)+1` for gap two, so `#pulls<=2 max h_i` | **Uniform reduction plus frozen evidence:** the temporal depth recurrence and ray stabilization are proved. The weaker one-chain bound `h<=#initial features through its root` passes 4,115,168 new held-out queues but is not proved. Fixed-factor pull potentials through width five are killed. The actual-right `00000` prohibition makes exclusion of eventual pull gaps `{2,3,4,5}` a second sufficient target. Either single-chain lemma would close nonconstant period two |
