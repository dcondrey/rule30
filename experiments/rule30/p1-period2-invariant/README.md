...
| Dual colex, phase gaps, and pull ancestry | Both inherited scans strictly colex-descend; the phase-gap cover is proved for all lengths. Every nonfirst retreat has one productive pull, and every post-retreat pull is exactly `102->0021` on `p-t=N-3`. The appended-coordinate parent forest obeys `h_i=h_(i-1)+1` for pull gaps at least three and `h_i=h_(i-2)+1` for gap two, so `#pulls<=2 max h_i` | **Uniform reduction plus frozen evidence:** the temporal depth recurrence and ray stabilization are proved. The one-chain bound passes 4,115,168 held-out queues. Its stronger marked form, `pull-depth+[raw=3]<=initial feature reserve`, passes 1,697,191 newly seeded queues and reduces the gap to preservation of one forbidden marked state. Neither is proved. Fixed-factor pull potentials through width five are killed; eventual actual pull gaps `{2,3,4,5}` give a second sufficient target |
