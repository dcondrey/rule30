...
| Mixed budget, origin-prefix Hall, and exact pull rewrites | `PREREGISTRATION-MIXED-RUN-RETREAT-BUDGET.md`; `PREREGISTRATION-ORIGIN-PREFIX-HALL.md`; `RESULTS-MIXED-ORIGIN-RETREAT.md`; `constant_tail_mixed_run_budget.py`; `constant_tail_origin_prefix_hall.py`; `constant_tail_pull_rewrite.py` |
...
| Dual colex, phase gaps, and pull origins | Both inherited scans strictly colex-descend; the phase-gap cover is proved for all lengths. Equality after the rightmost pivot leaves exactly `1->02`, `1->21`, or `10^m2->0^(m+1)21`, so every nonfirst retreat has one productive pull | **Uniform reduction plus frozen evidence:** the mixed budget `ret<=#1+#zero-runs` and stronger colex-origin prefix Hall claim pass 4,115,170 held-out queues. Additive ranks through width four are killed. Proving pull-origin Hall would give `d(r)>=r` and close nonconstant period two; it remains open |
