...
| Mixed budget, origin-prefix Hall, exact pull rewrites, and pull ray | `PREREGISTRATION-MIXED-RUN-RETREAT-BUDGET.md`; `PREREGISTRATION-ORIGIN-PREFIX-HALL.md`; `PREREGISTRATION-PULL-RAY.md`; `RESULTS-MIXED-ORIGIN-RETREAT.md`; `RESULTS-PULL-RAY.md`; `constant_tail_mixed_run_budget.py`; `constant_tail_origin_prefix_hall.py`; `constant_tail_pull_rewrite.py`; `constant_tail_pull_ray.py` |
...
| Dual colex, phase gaps, and pull origins | Both inherited scans strictly colex-descend; the phase-gap cover is proved for all lengths. Equality after the rightmost pivot leaves exactly `1->02`, `1->21`, or `10^m2->0^(m+1)21`, so every nonfirst retreat has one productive pull. Every post-retreat pull is exactly `102->0021` on the ray `p-t=N-3` | **Uniform reduction plus frozen evidence:** the mixed budget, colex-origin Hall, and weaker bottom-feature ray crossing claim pass 4,115,170 held-out queues. Ray stabilization is proved; its crossing count is not. Additive ranks through width four are killed. Proving the ray crossing lemma would give `d(r)>=r` and close nonconstant period two |
