...
| Dual colex, phase gaps, and pull ancestry | Both inherited scans strictly colex-descend; the phase-gap cover is proved for all lengths. Every nonfirst retreat has one productive pull, and every post-retreat pull is exactly `102->0021` on `p-t=N-3`. The appended-coordinate parent forest obeys `h_i=h_(i-1)+1` for pull gaps at least three and `h_i=h_(i-2)+1` for gap two, so `#pulls<=2 max h_i` | **Feature budget falsified; geometric bridge live:** a sparse length-77 queue has depth 3 from feature budget 2, killing both feature-depth and raw-reserve claims. The replacement `r>=2h-1` counts the zero coordinates discarded by that projection and matches the zero-prefix scale budget; it passed 5,272,917 held-out queues but is unproved. The counterexample is not hard-core endpoint-derived, so a restricted theorem also remains possible. Eventual actual pull gaps `{2,3,4,5}` give a second sufficient target |
