...
| Dual colex, phase gaps, and pull ancestry | Both inherited scans strictly colex-descend; the phase-gap cover is proved for all lengths. Every nonfirst retreat has one productive pull, and every post-retreat pull is exactly `102->0021` on `p-t=N-3`. The appended-coordinate parent forest obeys `h_i=h_(i-1)+1` for pull gaps at least three and `h_i=h_(i-2)+1` for gap two, so `#pulls<=2 max h_i` | **Uniform reduction plus frozen evidence:** the temporal depth recurrence and ray stabilization are proved. The marked form `pull-depth+[raw=3]<=initial feature reserve` passes 1,697,191 held-out queues. Base rows and appended children preserve it uniformly; only inherited zero-reserve cells remain, with three possible bad table cases strictly left of the pivot. Their recursive positive-reserve `D8` blocks are not yet controlled. Eventual actual pull gaps `{2,3,4,5}` give a second sufficient target |
