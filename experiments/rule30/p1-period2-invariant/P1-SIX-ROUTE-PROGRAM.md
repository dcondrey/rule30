...
| Queue ancestry / distance | Prove `d(r)->infinity`, equivalently constant-frontier orbit disjointness from infinite hard-core terminals | **PULL RAY AND TEMPORAL ANCESTRY PROVED; FEATURE MARK KILLED, COORDINATE INTERVAL LIVE**. A sparse length-77 queue gives depth 3 from feature budget 2, falsifying both feature-depth and raw-reserve claims. The replacement `r>=2h-1` passed 5,272,917 held-out queues and aligns with the zero-prefix token budget | Prove a nested `2h-1`-coordinate interval for every depth-`h` pull, or prove the corresponding zero-prefix greedy lemma. Alternatively restrict to hard-core endpoint-derived queues or exclude unbounded depth under eventual actual pull gaps `{2,3,4,5}`. Fixed-factor ranks through width five are killed |
