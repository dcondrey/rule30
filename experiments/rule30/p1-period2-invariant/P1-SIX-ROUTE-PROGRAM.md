...
| Queue ancestry / distance | Prove `d(r)->infinity`, equivalently constant-frontier orbit disjointness from infinite hard-core terminals | **PHASE-GAP COVER PROVED**; every eventless retreat is first, and all other retreats inject into disjoint two-row zero-block windows | Prove repeated creation of the matched phase gaps requires unbounded initial ancestry. `#1+1`, fan-out-only cover, and scalar/log gap ranks are killed |
