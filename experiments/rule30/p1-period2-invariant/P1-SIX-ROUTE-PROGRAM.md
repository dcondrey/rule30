...
| Queue ancestry / distance | Prove `d(r)->infinity`, equivalently constant-frontier orbit disjointness from infinite hard-core terminals | **PULL RAY AND TEMPORAL ANCESTRY PROVED; UNRESTRICTED FEATURE AND COORDINATE MARKS KILLED; ENDPOINT-DERIVED ROOTS LOCALIZED**. `3001 0^382 2` reaches depth 3 from root 3 and `m=390` reaches depth 4, but the hard-core `203` boundary identity forces every applicable root into the last three initial coordinates | Prove a depth bound for those three endpoint-derived roots, a scale-aware gap-vector descent, or the eventual-gap `{2,3,4,5}` syndetic exclusion. Fixed-factor ranks through width five are killed |
