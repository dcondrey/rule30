...
| Queue ancestry / distance | Prove `d(r)->infinity`, equivalently constant-frontier orbit disjointness from infinite hard-core terminals | **PULL RAY AND TEMPORAL ANCESTRY PROVED; ONE CHAIN LIVE**. Every nonfirst retreat pairs with a productive pull, all post-retreat pulls lie on `p-t=N-3`, and pull depths satisfy `h_i>=floor(i/2)+1`. The one-chain feature bound passes 4,115,168 newly seeded held-out queues | Prove that a pull ancestry chain rooted at `r` has depth at most the number of initial `1`/zero-run features through `r`; this already gives `#pulls<=2|R|`. For the actual period-two application, it also suffices to exclude eventual pull gaps `{2,3,4,5}`, using the actual-right `00000` prohibition. Fixed-factor ranks through width five are killed |
