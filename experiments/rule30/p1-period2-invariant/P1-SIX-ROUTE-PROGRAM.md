...
| Queue ancestry / distance | Prove `d(r)->infinity`, equivalently constant-frontier orbit disjointness from infinite hard-core terminals | **PULL RAY AND TEMPORAL ANCESTRY PROVED; ONE INHERITED MARKED STATE LIVE**. Pull depths satisfy `h_i>=floor(i/2)+1`. The stronger exclusion of raw state `3` at zero remaining reserve passes 1,697,191 held-out queues. Base rows and appended children preserve it uniformly; a first inherited failure must be left of the colex pivot in one of three table cases | Prove the inherited zero-reserve lemma across recursively nested positive-reserve `D8` blocks; this gives `#pulls<=2|R|`. Alternatively exclude unbounded depth under eventual actual pull gaps `{2,3,4,5}`. Fixed-factor ranks through width five are killed |
