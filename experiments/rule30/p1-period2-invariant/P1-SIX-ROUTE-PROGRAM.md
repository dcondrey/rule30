...
| Queue ancestry / distance | Prove `d(r)->infinity`, equivalently constant-frontier orbit disjointness from infinite hard-core terminals | **LIVE**; phase-labelled zero-gap retreat matching passes a registered 460,123-queue held-out corpus | Prove the matching by two-row interval Hall inequalities, then bound repeated creation of matched phase gaps. `#1+1` and fan-out-only cover are killed |
| Dyadic hard-core lock | Prove every ultimately dyadic cut with an infinite hard-core endpoint is the exceptional eventually-`2` family, then apply reachability separation | **LIVE** | Control infinitely many separated period doublings with lasso entry state retained |
| Peel-recursive Craig induction | Express the next separator as a Peel pullback of its predecessor plus an exact restart/boundary state | **LIVE** | Test transition closure of the restart annotation before evolving formulas |
| Cartier `S(0)` rationality | Prove the center generating series is nonrational, equivalently the center is not eventually periodic | **LIVE**, substantially stronger automaticity shortcuts separated | Seek a sequence-specific unbounded residual distinction; finite prefixes and generic Ore-height ladders are closed |
