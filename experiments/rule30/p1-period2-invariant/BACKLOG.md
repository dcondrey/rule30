...
| L11-RW-TERMINAL-DEFECT | `|H_r(n)| / 2^n` (the hard-core + `12a`-terminal RW candidate population) decays with `n`, `r = 0,1,2` | motivates a shrinking-population argument for RW where `(BWH+)`'s full-degree obstruction blocks the unrestricted version | exhaustive `H_r(n)` count vs `2^n`, both `c`, `r = 0,1,2` | S1'/S2 done and empty (`RESULTS-PSI-ANCESTRY-R-EXTENSION.md`). Section 4 measurement run (`RESULTS-RW-TERMINAL-DEFECT-H-POPULATION.md`, `rw_population_h.py`): `|H_r(n)| = 0` exactly for `n = 1..16`, every `r`, both `c`. Two candidate proof shapes tried and falsified: (1) entrywise domination of the per-row transition counts by a null matrix `M` (`RESULTS-TRANSFER-DOMINATION-CHECK.md`) — false pointwise, though `M` matches the true mean transition rate almost exactly (~0.25 per entry, matching the fitted ~0.40 decay rate); (2) a bounded-suffix / saturating-max-survival-row hypothesis, falsified directly at n=17,18 (max survival row keeps growing: 5,9,10,10,11,12 on the sampled grid n=10,12,14,16,17,18, not saturating). Live candidate: `RESULTS-EXTINCTION-MARGIN.md` — the *gap* between target row count (`n+2`) and max survival row at `r=0` does not shrink toward 0; if it is bounded below by any positive constant for all n, `H_r(n)=0` follows directly, no statistics needed. Extended range (`RESULTS-EXTINCTION-MARGIN-AUDIT.md`, cross-checked against `docs/rule30/overnight/LEDGER-20260905.md` where the same quantity is called `gamma`): `c=2` gaps are 7,5,6,8,8,8 on the sampled grid n=10,12,14,16,17,18, then 10,11,10,12,9,12 for n=19,20,21,22,23,24, and 14 at n=30 — **the apparent plateau at 8 ends at n=19** and the gap widens rather than stabilizing; `c=3` runs 5,6,8,8,9,9 then 8,8,7,10,9,10 on the same two grids; the global minimum over every measured cell is 5, attained twice — `c=3` at n=10 and `c=2` at n=12 — which are the binding cells any `gap(n) >= g_min > 0` argument has to survive. The mechanism is a max-survival row growing with least-squares slope **0.726** on the six n=10..18 points and **0.561** over the full n=10..24 grid (`c=3`: 0.484 / 0.731) against a target growing at exactly 1 — not the "slope close to 1" that `RESULTS-EXTINCTION-MARGIN.md` states. Descriptive, un-preregistered data (there is no E1-E5 preregistration; see the audit §1); measured only to n<=30, obstruction H applies; not proven | full-degree `Delta` (obstruction, not kill) | `PREREGISTRATION-RW-FORCED-TERMINAL-DEFECT.md`; fires the capsule section 7 fallback; the collapse lemma (RW witness existence = `literal_extension` surviving hard-core+`12a`) is resolved by reading the code, no run needed. Direct row-permutation derivation attempted (`RESULTS-EXTINCTION-MARGIN.md` derivation section): `literal_extension`'s backward-solve chain factors through exactly the 8-element D4 group generated by `FORWARD[swap(s)]`, `s=0..3` — confirmed to be the *same* D8 affine action already fully used in capsule section 3, not a new reduction. Dead end as a generic argument; would need a specific word-family periodicity (like `endpoint_restart_cocycle.py`'s `2^m p_r`), none found. Do not retry the generic finite-monoid argument without a candidate family. 2026-09-05: `RESULTS-EXTINCTION-MARGIN.md`'s remaining structural premise is also gone. It claims the max-survival *source words* share a long common suffix, differing mainly in their first one or two symbols, which is the sole motivation for its "suffix window near the append point" proof route. `RESULTS-E3-MAXSURVIVAL-WORD-STRUCTURE.md` disconfirms it on all 18 cells n=10..18, both tails, recomputed: on c=2 the free prefix length runs 6,2,1,13,5,14,3,6,18 for n=10..18, so the claim holds only at n=11,12, and at n=18 c=2 (the extinction document's own last cited point) the 16 finalists share no common suffix at all. Combined with `RESULTS-FIBER-EXTREMAL-FAMILY.md` section 3, which finds no parametrized family on the continuation side either, E4 has no family to exclude. The gap data itself is unaffected. Do not attempt step 1 of `RESULTS-EXTINCTION-MARGIN.md` in its stated form |
