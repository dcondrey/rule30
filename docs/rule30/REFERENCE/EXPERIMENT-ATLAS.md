...
Updated: 2026-09-11 (27 new uncommitted RESULTS and experiments pending integration)

Status: **ORGANIZATIONAL SYNTHESIS; NO PRIZE PROBLEM SOLVED.** (Matrix staleness: new p2, quarter-wave, defect, and duhamel results pending full categorization)
...
| Fixed-period automata/SAT | `PATH:7,31--32,41`; ladder reports rung 0/1/2 | Many exact bounded exclusions (`C`); plus one *uniform* limitation: the ladder's mode (ii) can never return EMPTY for `w=01` at any depth for any `q` not a multiple of 420 (`U`, from four finite tori, `RESULTS-ladder-rung2-periodic-realizability.md`) | Witnesses and minimal cores; four explicit alternating-centre Rule 30 tori `(N,T) = (7,4),(84,6),(155,10),(728,14)` and their exact period census | State/free-boundary growth; finite depth is not uniformity; and for the residual `q` a witness must be time-aperiodic, which no finite certificate can supply |
...
| Reversibility, second-order lifts, and the spacetime transpose | `PATH:100`; `RESULTS-second-order-reversible-lift.md`; `experiments/rule30/column-transpose/` | The Fredkin lift of Rule 30 (NKS **rule 30R**) from the lone seed has centre column identically `1`, by induction on the closed left cone (`U`); lifted and unlifted orbits agree only at `t=0,1` (`U`); the pair CA `G'` of the space-direction column automaton has image exactly the SFT `{(v,w) : v[t]=1 => w[t] != v[t+1]}`, `4*3^(h-1)` of `4^h`, hence is **not surjective** and fails Kari-Taati's transpose hypothesis (`U/K`, exhaustive to `h=9`) | The 30R construction and its reversibility control; a cross-rule control table (90 period 2, 110/150 period 3, 45 no period `<=8` in 201 symbols) showing the lift *can* carry an aperiodic centre; Kari-Taati Thm 52 spacetime-transpose bijection as the correct form of the same instinct | Obstruction E in its reversible-dynamics instance: a conservation law proved on the lift constrains the lift's orbit, not Rule 30's, and a finite conserved structure carries `O(1)` bits about an infinite orbit (`FINDINGS:86`) |
...
| Ordered-region structure | `RESULTS-ordered-wedge-glide.md` | Left-frame recurrence with absorbing OR; glide form of the left region's periodicity; NKS p. 871 depth table reproduced to the unit, first period 32 at `j=87,867` (`C/M`) | The recurrence pair, the glide form connecting the region to rung-2's tori, the rule 90 split, and a corrected obstruction A | Prior art for the phenomenon (NKS p. 871). Still an unbounded-period requirement at depth `Theta(t)`; the conditional route first proposed here is retracted, section 5 |
...
| `RESULTS-SYNTACTIC-MONOID-QUOTIENT.md` | `K` | The finer congruence that keeps the full intermediate scan trace (not just its final `D8` element, which the no-go rules out) shows zero class merges through `L=18` (3,015,168 exhaustive legal words) | Closed by `RESULTS-SYNTACTIC-MONOID-INJECTIVITY-PROOF.md`: injective for all `L`, no stabilization point exists |
| `RESULTS-SYNTACTIC-MONOID-INJECTIVITY-PROOF.md` | `U/K` | `sig_A` is injective on same-length words for **every** `L`, from two finite checks on the action table (each symbol acts as a permutation; distinct normalized symbols act differently somewhere), so the block right congruence is the identity and the block monoid is free on `{0,1,2}` — no finite-state lossless summary of an interior block exists at any radius. Legality is unused; two entering states already suffice (minimal sets `{0,2},{0,3},{1,2},{1,3}`); `delta_1 == delta_3` is why `SYMBOL_QUOTIENT` is needed | Lossy summaries adequate for a weaker downstream target, and summaries of the whole queue rather than an isolated interior block |
| `RESULTS-PAIRED-DIVERGENCE-INVARIANT.md` | `K` | Paired-word (product-automaton) divergence: tail-3 divergence width grows without saturation as the family parameter grows; tail-2 pairs die simultaneously before divergence can develop | Negative for both tested constructions; the general paired/relative-invariant question stays open |
| `RESULTS-LYAPUNOV-POTENTIAL-SEARCH.md` | `K` | All tested extensive-count and density candidate potentials are artifacts of the queue's fixed +1-symbol-per-step growth, not dynamical signal; one bounded recurrence-gap candidate is falsified by a genuine 5-step drift | A monotone potential not reducible to a length-growth artifact, if one exists |
| `RESULTS-ZERO-PREFIX-LEADING-TERM-EXTEND.md` | `K` | Extends the zero-prefix greedy lemma's (`RESULTS-SCALE-TELESCOPING.md`) exhaustive horizon from length 22 to 23 and adds structured-adversarial and random coverage to length 2000, zero failures | Superseded as the proof target by `RESULTS-ZERO-PREFIX-GREEDY-REDUCTION.md`; its large-`n` random axis is weak evidence, the continuation dies within a row or two there |
| `RESULTS-ZERO-PREFIX-GREEDY-REDUCTION.md` | `R/K` | The greedy lemma is more than `(1)` needs: the per-row claim `max S_j >= j` on legal rows implies `s_2 <= n` and `s_3 <= n+1` in two lines, is strictly implied by the greedy lemma, and holds on all 44,205 legal rows through length 20. Equivalently, some suffix of `W` of length `<= n-j` moves the row-`j` affine off its all-zero value. Five candidate strengthenings killed, including the natural propagation argument `A_(j,j) != A_(j,n)` | A proof of `max S_j >= j`; it is tight only on `W in {121, 221}`, tail 3, row 2, the two words attaining `(1)` |
| `RESULTS-C3-LOCALIZATION-NO-GO.md` | `K/R` | C3 (44,205 rows to n=20) and C3ab (72,351 rows to n=21) hold but do not localize. The abelianization of the newest-cut group is exactly the existing `(alpha,beta)` with `gamma` the kernel; two bits is the exact size. The freeze set `K` is nonempty, sparse and never legal, but the freeze is `P`-independent while survival is `P`-dependent, and no shared quantity exists. Ten local shapes killed with witnesses | A quantity shared between the nested chain of `V` and the lifetime of `P.V`; `(1)` itself is again the target |
...
| `RESULTS-BINARY-WEDGE-HORIZON.md` | `C/M` | Exhaustive census of `M_c(n)` over all `2^n` sources, `7 <= n <= 29`, both tails, zero `(BWH+)` failures; the preregistered sharp `M_c(n)<=n` is falsified at `M_3(15)=16`; the slack `g(n)-n` falls at `-0.129` per unit `n`, so the bound is not asymptotically tight and a lossy `g(n)<=0.95 n` plus the census below `N_0<=30` would close the rung, though the fitted slope has risen at every extension and the margin is down to `0.079`; the maximum is attained by 4 to 64 sources sharing one or two forced continuations | `(BWH+)` itself is unproved, and no finite census can supply it. The nested-continuation route is closed: `Q(n+1,c)=Q(n,c)[2:]` fails 13 of 18 consecutive same-tail steps over `18<=n<=27`, and the surviving converse (it holds whenever `M` drops by exactly two) has five supporting steps and no mechanism |
...
### 7.1b Every `K` in the register is a contraction; every `U` is an identity

Sorting the register by evidence level rather than by topic splits it exactly.
The `U` results are identities, reductions, or counts: zero-tail
classification, rotated Peel, rank-zero reduction, `sig_A` injectivity, rung-2
realizability.  The `K` results are, without exception, attempts to make some
quantity decrease.  Section 7.1's four-state kernel is why: the actions are
permutations, so nothing is forgotten, so nothing contracts — and
`RESULTS-SYNTACTIC-MONOID-INJECTIVITY-PROOF.md` turns that into a proof that no
lossless bounded summary exists at any radius.

The operational form is the contraction screen in `START-HERE.md`.  Its
positive half matters more: injectivity is a supply of exact size, so the
technique it recommends is counting, not monotonicity.  Full argument and the
fifteen-experiment tally in `CROSS-ARCHIVE-2026-09-07.md` section 1.
...
### E'. The ordered region — measured, and mostly prior art

`RESULTS-ordered-wedge-glide.md`.  The register had never taken the ordered
left region as an object; it now has an exact recurrence, a glide form, and a
rule 90 split.  The phenomenon itself is NKS p. 871 and the archive should
treat it as such.

Two proposals from the first draft are **withdrawn**, and the withdrawals are
the useful content: "left period is 16 for every `j`" is false at `j=87,867`,
and the width-two contradiction built on it does not close, because the
composition stays inside the ordered region and the aperiodicity it invoked was
measured on the seed, which does not satisfy the hypothesis.  Kopra 2023
Thm 3.5 stays in the chain.

What is left to run, in order: (i) the synthetic check — do the five `T=24`
on-cycle rung-2 witnesses have ordered regions with spatial period `16p`?  That
is the only thing that would give the composition content; (ii) the left-frame
doubling law, which is a real open question about the tame cone and is
sketched in an NKS parenthetical.  Neither is P1.

Kill condition for (i): the witnesses' ordered regions are spatially periodic,
which would show the composition proves nothing.
