...
0. **One cheap synthetic check**, added 2026-09-07, placed first only because it
   is an hour's work and closes a question rather than opening one: do the five
   `T = 24` on-cycle rung-2 witnesses — ready-made width-two periodic Rule 30
   diagrams — have ordered regions with spatial period `16p`? Either answer
   settles whether the composition argument of
   `RESULTS-ordered-wedge-glide.md` section 5 has any content. Read that
   report's sections 5 and 6 first: its first draft proposed two lemmas that
   are withdrawn, one of them false (the left-frame period is not 16 for all
   `j`; it doubles at `j = 87,867`, NKS p. 871), and the report's live target is
   now the left-frame doubling law, which is not PP1.
