...
**R7 correction, 2026-09-09:** row 7's original a7 i.o. obligation is
already implied by the proved five-zero theorem. It gives odd-q recurrence,
not aperiodicity or the residual even-q claim. The finite T<=24 torus
census does not force remaining witnesses to be aperiodic. The pinned
torus splice also needs a one-letter seam repair. The existing q coverage
is preserved; no period-two theorem is claimed. See
[the verified rung-3 audit](RESULTS-ladder-rung3-io-and-aperiodicity-audit.md),
which supersedes those readings of row 7.
...
If it moves by `O(1/W)` or less in the window width `W`, its limiting value
cannot distinguish these two arbitrary arrays. **Scope correction
(2026-09-15):** the overwritten array need not satisfy Rule 30. This test
therefore excludes inferring P1/P2 from the bulk statistic alone; it does
not exclude a dynamical identity or a quantitative bound connecting the
statistic to the center on valid seed orbits. Nor does it exclude using the
finite-width discrepancy at a specified rate. The individual geometric
proposals below supplied no such connection.
...
*The left boundary is quantitatively different and R4 does not cover it.*
Measured 2026-09-07 (`RESULTS-ordered-wedge-glide.md`): the left light-cone
frame is periodic to depth `3, 8, 29, 400, 87867, >2.1e9` for periods
`2,4,8,16,32,64`, which is NKS p. 871 reproduced here to the unit, against the
right frame's `2.4 log2 t`.  The left reaches vastly deeper per unit of period,
and it passes the rule 90 control, which the left-frame XOR does not survive.
It is nevertheless still an unbounded-period requirement at depth `Theta(t)`,
so R4's verdict carries over in kind if not in rate.  No route follows: the
conditional argument first proposed there is retracted in section 5 of that
report.
...
aperiodic `col_{-1}` at every `R` — is not.  That was the next target.

**RUNG 2 RESULT (2026-09-07): realizability PROVED for `w=01` at every `R` and
`k`, for every `q` that is not a multiple of 420.  See
`RESULTS-ladder-rung2-periodic-realizability.md`.**  The witnesses are four
explicit `N x T` tori — spatially *and* temporally periodic Rule 30 diagrams,
`(N,T) = (7,4), (84,6), (155,10), (728,14)` — whose `col_0` is exactly
`01`-alternating and whose `col_{-1}` has minimal temporal period 4, 6, 10, 14.
On a torus "not eventually `q`-periodic" is arithmetic on one integer
(`p` does not divide `q`), which is why a finite object settles an infinite
quantifier here where a7's `X(k,01)` could not.  Splicing the lone-seed prefix
in front of a torus's letters passes every constraint of the *unmodified*
engine at `R=1..12`, `k=1..4` (0 rejections, 0 derived-column mismatches, 0 pin
violations), and Lemma R3 makes that uniform in `R` and `k`.  Two structural
lemmas: rightward extension is an `L_T`-preimage (= the pin), and iterated
images of a function converge to its cycles, so a time-periodic pair extends
rightward at *every* depth iff it is spatially periodic too.
**Limits, stated:** the achievable `col_{-1}` periods are exactly
`{4,6,10,14}` for every even `T <= 24` (exhaustive to `T=12`, every
candidate resolved to `T=24`, zero undecided), so `q` divisible by
`lcm = 420` is out of reach; a pre-registered "2 x prime" prediction that
would have made the family unbounded was refuted at `T=22`.  Controls both
fire correctly: the `p=1` case is killed by a one-line pin argument
(Lemma C1), reproducing rung 0's EMPTY, while for Rule 90 the same map is a
bijection and every period is free — the OR is the whole difference.
Consequence for whoever continues: an escape at `q = 0 mod 420` cannot be
time-periodic at all, so it must be exactly a7's aperiodic object, i.e. the
same zero-set question as R1.
...
**DIAGNOSTIC RESULT (first run 2026-08-30, re-run on protocol 2026-09-16):
route stays alive; no theorem.  See `RESULTS-orbit-closure-diagnostic.md`.**
`T = 2,000,000`, the registered target length; the first run used
`T = 1,100,000` off protocol and is superseded, see that document's
deviation-and-correction section.  Block-frequency sup-deviations decay with
slope `-0.473` to `-0.581` in `log2 W` for `L = 1..12`, i.e. the `W^{-1/2}`
law, and sit inside the 20-seed i.i.d. Bernoulli(1/2) band at essentially
every window and depth (2 exceedances in 25 cells, at `L=2, W=2^14` and
`L=4, W=2^12`, against a design expectation of about 1.2).  None of the three
pre-registered disconfirming criteria fired: no deep-window drift, no
depth-dependent plateau, no exceedance cluster.  Rule 90 control PASSED at
every cell, saturating the exact `1 - 2^-L` value, so the pipeline detects the
failure mode it was built to detect.  Verdict as pre-registered: CONSISTENT
WITH unique ergodicity, which proves nothing.  The route is not killed and not
advanced.
...
integrates row 46's existing generic refutation and narrows two standing
screens to their proved domains. It also records the quantitative
origin-preserving R1 mismatch bound and a sparse sufficient P2 correlation
condition. Neither missing singleton estimate is proved. Historical KILLED
labels concern the stated candidates and certificate classes, not every
argument using the named mathematical subject.

The [P1 mismatch follow-up](RESULTS-r1-mismatch-followup.md) gives an exact
right-mismatch/centre-error budget, a Rule 90 sharpness control, and a
three-prefix `0001` driven-boundary invariant. It obtains no right-mismatch
upper bound. It also corrects the old pin-lock sampler's prefix coverage
and its finite-window survivor label.

The existing 124- and 104-cell periodic cylinders also imply
[sharp finite-left deadlines](RESULTS-r1-cylinder-deadlines.md): alternating
centre agreement fails by `d+7` and `d+10`, respectively, for any row with
the specified right prefix and no ones left of `-d`. This is uniform in
left depth and arbitrary right continuation, but only for those prefix
families; no singleton entry theorem is known.

The [fundamental-frequency audit](RESULTS-fundamental-frequency-search.md)
extends the exact spatial quarter-wave identity with a classification of
bounded one-sided linear extraction weights. It records finite tests of
fixed temporal frequencies and held-out peaks; no temporal line or
asymptotic cancellation theorem is established. This does not reopen
row 14's specific low-degree FFT shortcut.
The [bilateral-current continuation](RESULTS-bilateral-frequency-current.md)
keeps the nonlinear source and bounds its spatial truncation explicitly;
the derived optimal frequency is a kernel parameter, not a proved center
tone. The [1/3 scale target](RESULTS-third-frequency-scale-target.md) would
prove P1 through unbounded sublinear Fourier growth, with the required
seed estimate still open. A [small current correction](RESULTS-quarter-wave-current-collision.md)
is refuted by two exact transitions on the actual seed.
...
| 94 | (Lock): 0-initial right half-line driven by a periodic centre forces `col_1` periodic | 1 | **KILLED as a lemma; the implication survives, the hypothesis does not** | Correct and genuinely new *object*: since Rule 30 needs only `x_0` as the left boundary of the half-line `i >= 1`, the seed's whole right half-plane is a deterministic function of the centre column -- no fiber, no free boundary, so obstruction F does not apply, and it is not `RESULTS-alt-trace-fiber.md` (which drives leftward and enumerates right halves). Validated bit-exactly against the true lone seed. `(Lock) + Kopra 2023 Thm 3.5 => P1` is a valid inference. But (Lock) is false as a driven-half-line statement: 58 of 69 primitive drives `p <= 8` leave `col_1` with no period `<= Qmax`, `w = 01` included, stable to `T = 100,000`, `Qmax = 20,000`; only 11 sparse locks at `p in {3,4,8}`. The proposed OR-sticky/NOR-reset attractor mechanism dies twice: rule 90 does not lock either (0 of 21), and the **actual Rule 90 lone seed** has `col_0` eventually periodic with `col_1` aperiodic to 20,000 -- a real orbit in exactly the configuration (Lock) forbids. The `w = 01` case is a7's `X(k,01)` i.o. lemma in cleaner coordinates; independent cross-validation, zero-set density 0.2145 here vs a7's 0.2132-0.2168. Seed-specific (Lock) is NOT refuted (a periodic word need not be a realisable centre column, recorded pre-run); what died is this route to proving it. Follow-on, run and closed the same day: the width-2 check on the five `T=24` on-cycle rung-2 witnesses shows all five share one alternating `U` with five distinct `V`, so a periodic `col_0` does not determine `col_1` even at infinite depth -- the uniqueness route to any (Lock)-shaped arrow is dead, and `NEXT-DIRECTIONS-post-rung2.md` section 3 is struck. Those witnesses also have eventually periodic width-2 traces, so Kopra 2023 Thm 3.5 certifies them **outside `L_0`**: they bound the ladder, not the seed. | `RESULTS-lock-driven-halfline.md`; `experiments/rule30/lock-driven-halfline/`; `NEXT-DIRECTIONS-post-rung2.md` sec. 3 (struck) |
| 95 | The 4-cell unit (3 cells plus the one below their centre) as the primitive object: read the diagram as a 2D subshift of finite type on 8 tiles instead of row by row | 1 | **KILLED as a method; one fact retained** | The frame is legitimate and was absent (`docs/` census: zero hits for subshift of finite type / tiling / tetromino; ARM7's "spacetime tiles" are quadtree blocks in a P3 probe, not the local constraint). Retained fact: exactly two of the four sites are determined by the other three -- `d` by the rule, `a` by left permutivity -- and the two failures are precisely the OR's arguments, which locates Rule 30's information loss frame-independently. The Rule 90 control sharpens rather than passes: rule 90's tile never reads `b`, so its undetermined site is a dummy and its tile set is a product with a free coordinate, whereas Rule 30 reads all four and is still non-invertible at two. What dies: the sublattice reading. `a`, `c`, `d` share one parity of `t+x` and `b` has the other, giving the exact dichotomy `c=1 => d = NOT a` (autonomous) versus `c=0 => d = a XOR b` (coupled), which is the Eloranta-Nummelin Rule 18 kink method's entry point; but the coupled branch fires at 0.4995 of interior sites, flat to within 0.004 in every `x/t` bucket across the whole cone, with autonomous runs of mean length 2.02. No background, no defect gas, no sparse regime anywhere. Computationally the frame is the ladder: a transfer matrix on width-`R` strips of this SFT is route R7 and inherits obstruction F. The SFT is deterministic downward, so the tiling literature's leverage (undecidability, aperiodic tile sets) does not apply.  Follow-ons settled in the same arm: no rotation of the unit is a symmetry (orbit of 30 under <mirror, complement> is {30,86,135,149}, fixed by neither generator), so rotating the rule always leaves rule 30 -- obstruction B in its purest form; and all four proposed switching schedules (alternate each step, every 4, on the centre value, on hitting 0) keep the centre aperiodic to T=20,000. The light-ray/weak-projection reading names the existing fiber object and its kernel is exactly the two non-invertible sites above. | `RESULTS-spacetime-tiling.md`; `experiments/rule30/spacetime-tiling/` |
| 7 | R7, omega-automaton periodicity ladder: one exclusion theorem per period `p` | 1 | OPEN | Reclassified from STALLED 2026-09-05: blocked on one named ingredient, not paused for resources. Calibration passed and `Thm(1)` mechanised; open cases `p=2..8` NONEMPTY with verified witnesses; depth, the boundary pin, and `Q` are all proved/measured unable to close it (rung 1 Corollary 2/3: the pin buys strictly less than one column and cannot decide anything depth cannot; bisimulation classes grow only `~R^2.75` against `4^R` raw states, same rate with or without the pin). What remains, stated precisely by rung 1 sec. 6: realizability -- an infinite letter word, for every `R`, whose centre is eventually `01`-periodic and whose `col_{-1}` is not eventually periodic. Lemma 4 rules out the natural time-2-periodic construction (all 16 states collapse to one constant 2-cycle within 3 steps). This is obstruction F in its sharpest known form. Only one mode remains because the other already died: row 77 (second stream) proved the phase slip extends to a full half-plane, so ladder mode (i) can never return EMPTY at any depth -- "retires R7 mode (i) entirely" (`FINDINGS.md` row 77). Mode (ii), the untested inclusion mode, got no traction at smaller scale (`FINDINGS.md` row `a22_r7_mode2`) and is what "realizability" above refers to. **UPDATED 2026-09-07 (rung 2): realizability is now PROVED for `w=01` at every `R` and `k` and every `q` not a multiple of 420, from four explicit spatially-and-temporally periodic tori `(N,T) = (7,4), (84,6), (155,10), (728,14)` with `col_{-1}` of minimal period 4, 6, 10, 14; so mode (ii) can never return EMPTY at those `q`, in particular at `q=1`, the value rungs 0 and 1 actually ran.  Lemma 4 is respected: these diagrams are time-periodic but the *centre column alone* has period 2, not the diagram.  What is left of R7 is only `q` divisible by 420: the census of alternating-centre tori is exact and finite (`{4,6,10,14}` for every even `T <= 24`), the registered `2 x prime` prediction that would have removed the restriction was refuted at `T=22`, and a witness at those `q` must therefore be time-aperiodic — the same object as a7's unproved i.o. lemma and the same zero set as R1.** | `RESULTS-ladder-rung0.md`; `RESULTS-ladder-rung1.md`; `RESULTS-ladder-rung2-periodic-realizability.md`; `FINDINGS.md` rows 77, `a22_r7_mode2` |
| 8 | R8, invariant-measure classification of the vertical orbit closure | 2 | **OPEN** | Cheap disconfirming diagnostic ran; none of the three pre-registered criteria fired, Rule 90 control passed at every cell.  Verdict as pre-registered: "CONSISTENT WITH unique ergodicity, which proves nothing.  The route is not killed and not advanced." Conditional on row 79 (orbit-closure patch census): if the checkerboard temporal fixed point lies in `Y`, this route's target fails; row 79's unbounded-patch-growth question is the load-bearing open condition, not a separate route. | `PATH.md` R8; `RESULTS-orbit-closure-diagnostic.md`; status now conditional, see 9.3, row 79, and `RESULTS-checkerboard-growth-extended.md` |
...
| 12 | Arm 3: sealed WASM tournament for a sub-quadratic centre-column algorithm | 3 | KILLED | Reclassified from STALLED 2026-09-06. The sealed LLM tournament itself (a search for a *new* algorithm) was never run past n=4031 and still hasn't been -- that decision band needs the sandboxed harness, not a local script, and remains genuinely not enforced. But a local follow-up (`arm3_local_exponent_extend.py`, both candidates cross-checked against `center_column.py`) extended the *known*-algorithm calibration to n=1,000,031, past the pre-registered n>=10^6 threshold: naive fits alpha_hat=2.027 (R^2=0.9998), bit-parallel fits alpha_hat=1.983 over the full range (R^2=0.997). A naive tail-only refit (last 4 points) gave a low outlier (1.666); not reported as a finding -- it was measured while two unrelated CPU-bound jobs (row 44's Groebner saturation) were pinning both cores, a known confound, not rerun clean given those jobs' own runtime. No exponent improvement found at real scale, matching the pre-registered expected null.  This row's wasmtime fuel meter was later read and rejected for reuse by row 92: WASM's fixed-width ops make one instruction genuinely `O(1)`, which does not hold for OpenEvolve's Python search. | `PREREGISTRATION.md` Arm 3; `RESULTS-arm3-run1.md`; `arm3_local_exponent_extend.py`; row 92 |
...
| 18 | Arm 7: exact dyadic spacetime tile grammar (quadtree DAG, Morton order, compression ensemble) | 3 | KILLED | Reclassified from STALLED 2026-09-06: the candidate contract this row names as its unmet acceptance gate was already run through Crosstalk and closed as row 19 -- same context files (this doc plus its three probes), same acceptance criteria verbatim ("poly(log n) state and work bound, center-bit extraction, and a deterministic held-out falsification probe"), session `20260828T034736Z`, zero surviving candidates. Diagnostic content stands: DAG exponent 1.367 rising to ~1.48 over horizons 64-1024; tile perimeters carry `Theta(scale)` bits; every probe first constructs the full spacetime area. Checked before spending on a fresh Crosstalk run 2026-09-06 (Rust toolchain installed, `crosstalk` binary built specifically to attempt this): rerunning would have repeated row 19's exact prompt against the same files, which the project's own standard treats as unjustified without a new mechanism. | `ARM7-dyadic-spacetime-grammar.md`; row 19; `runs/rule30-arm7/report.md` |
...
| 26 | All-one trace fiber / constant-one case | 1 | **PROVED** | Theorem obtained: the all-one trace forces the checkerboard left half `L_k = 1` iff `k` positive even, which is infinite, so no finite row has constant-one trace.  With row 25 this excludes every eventually constant lone-seed centre.  Reproved in one line from the OR-latch pin, and rederived mechanically by the ladder at `p=1`.  The eventually-one case alone is not new: Jen 1986 Theorem 7a already excludes it for every finite Rule 30 configuration (row 84, packaged and checked componentwise in row 72) -- what this row adds is the explicit fiber, sharp horizon and extremizer count, not the bare exclusion. | `RESULTS-eventual-period.md`; `RESULTS-inverse-trace.md`; `PATH.md` §2; row 72, row 84 |
...
| 28 | Inverse-trace continuation: reconstruct the forced left half from a periodic trace and the actual right half | 1 | KILLED | Reclassified from STALLED 2026-09-05. The exact method (standard left permutivity, not a new theorem) produces only per-case contradictions, e.g. `T=1855,p=148` breaks at depth 166; no period-independent (uniform-in-`p`) contradiction found. A follow-on two-provider evolutionary search (`runs/rule30-inverse-trace`) aimed squarely at the missing uniform-tail obstruction returned nothing: zero of 72 extracted claims verified, two Lean artifacts contained `sorry`, final synthesis empty, `NOT_ESTABLISHED`. Its own writeup: "does not justify rerunning the same scientific prompt." Same missing closure as rows 32/46/7 (obstruction A: no `p`-uniform argument). | `RESULTS-inverse-trace.md`; `runs/rule30-inverse-trace` |
...
| 32 | Incremental SMT exhaustion of nonconstant periods (all rows to radius 8, periods to 6) | 1 | OPEN | Reclassified from STALLED 2026-09-05: not out of steam, precisely reduced. Exact horizon table (40 cells) plus three PROVED results (same-forward-orbit collision characterization = row 31; all-one fiber horizon; run-of-ones wedge). "Merely enlarging the SAT grid cannot prove it" -- the remaining obligation is exact: period-two reduces to a zero-stroboscopic obstruction for `F^2`, a radius-two CA, with two one-sided cases (support entirely right, or entirely left with even right boundary) already excluded; only mixed support, or one-sided-left with odd right boundary, remains. This is very likely the same open separator as Part III's (PT2)/needle conjecture under different variable names -- cross-check before opening new computation. | `RESULTS-eventual-period.md`; cf. row 31, row 75, sec:periodtwo |
| 33 | Right-cone branch-free bit-parallel reconstruction plus a solver-free bounded support certificate | 1 | KILLED | Reclassified from STALLED 2026-09-05: complete, not paused. 1,824 sweeps over 14.9M right parts show no descent (`min_deepest_one` stays within ~20 of `depth` for every `(p,W)`); the doc's own verdict is "do not extend the sweep." Its prescribed follow-on ran separately as row 34: the periodic region reachable from the boundary is only `~2.4 log2(t)` diagonals wide against a centre at diagonal `t` (obstruction A), which is why this representation cannot reach the centre column. Only new proved content: the three pinned right-cone diagonals. | `RESULTS-right-cone.md`; row 34 |
...
| 46 | Ore-relation ladder `S(0) <= S(1) <= ...` between P1 and non-2-automaticity of A051023 | 1 | **OPEN; generic Lemma L REFUTED** | `S(0) <=> P1`; all `S(n)` together are equivalent to nonautomaticity. A finite rank certificate proves only a bounded-height `S(n,d)`, not an all-height rung `S(n)`. The former generic polynomial-height / polylogarithmic-order obligation was already refuted by `a20_bridy_verification`; do not retry it. A separately proved Rule-30-specific conditional bound remains logically possible but unavailable. The later `a22` claim that such a contradiction proof is circular is incorrect. | `overnight/RESULTS-automaticity.md`; `AUDIT-automaticity-route-scope.md`; `experiments/overnight-arms/frontier_attack/a20_bridy_verification/bridy_verification.md` |
...
| 53 | Latent-space geometry to exact non-additive conserved window functions | 1,2 | KILLED | Run 2026-08-30, arm `a14_row53_conserved`: obstruction C (single-column blindness) at step 0, width-independently, over `Z`, `Z/2`, `Z/3` to `w=12`, cross-checked against rules 184/204/170/90/30. The ML part was a candidate generator only for the class killed wholesale, so no generator could rescue it; the exhaustive search is a byproduct confirming nothing was there, not the reason it died. Never merged into this register until 2026-09-05. | `overnight/TRIAGE.md` row 4; `experiments/overnight-arms/frontier_attack/a14_row53_conserved/row53_conserved_windows.md` |
...
| 71 | Settled-knowledge digest | none/infra | INFRA | Read-only survey listing what is proved, what is dead, and the pinned notation.  "Cite the source doc, not this file." | `overnight/CONTEXT-DIGEST.md` |
...
| 96 | Padded three-state, base-three paired-digit MSB automaton with output lookup periods 3 in time and 7 in space as an R1 counterdiagram class (right half-diagram `t>=0, x>=1`, eventually periodic centre, right neighbour aperiodic on the centre-zero set), eliminated by the exact endpoint-pair predicate plus SAT/DRUP over all axis tables | 1 | **KILLED as a construction class (C certificate, K on the stated class); R1 and P1 remain open** | Proved: "No padded three-state, base-three paired-digit automaton with output lookup periods three in time and seven in space can describe a Rule 30 right half-diagram whose centre is eventually periodic and whose right neighbour is aperiodic on the centre-zero set." The centre's eventual period and onset are unrestricted inputs; the arbitrary-radix period theorem (`RESULTS-r1-radix-automatic-period-bound.md`) supplies `H=27, P=162` for every state labelling, and an eventually periodic centre then agrees at shift 486 at every time `>= 81` as a consequence of the automaton structure, "not an externally imposed onset or period". The U reduction: centre eventually periodic iff `C[p,a]=C[p,b]` on every endpoint pair; under those equalities a masked-neighbour (`m_t=(1-c_t)r_t`) mismatch is exactly `C[p,a]=C[p,b]=0 and R[p,a]!=R[p,b]`, which is aperiodicity of the neighbour on centre-zero times. All 6,561 padded ternary axis tables covered with guards, "complete observation elimination for the specified class, without a finite witness horizon"; infinite initial support allowed, so the exclusion is of "this complete automatic ansatz for a generic diagram counterexample, rather than only excluding lone-seed or finite-support inputs". Run: 609 variables, seed 83,355 clauses with 84 actual Rule 30 gates and no early centre equations; 14,314 ordinary gate implicants imported from the narrower ternary test, every earlier centre cut and clock-based condition discarded; UNSAT in 30.79 s under a 60 s cap after 2,266 candidates and 64,716 new gate implicants, final 162,372 clauses. Independent verifier rebuilds every expected clause (40,979 endpoint pairs, 316,120 coordinate paths, 1,016,734 transition steps, 632,240 frozen truth-row cube comparisons), CNF matches exactly, independent DRUP rerun VERIFIED, SHA-256 of result/CNF/DRUP recorded. Rule 90 control `controls.rule90_control(6)` passes at periods 2, 4, 6; the generic gate checker accepts the binary Rule 90 diagram in its own radix and no ternary Rule 90 presentation is assumed. Scope, quoted: "The obstruction is specific to three core states, radix three, and the stated lookup periods. It does not bound the automaton size of general Rule 30 diagrams, exclude nonautomatic diagrams, or decide R1." A finite certificate refuting one complete, explicitly bounded representation class; not a theorem about Rule 30 diagrams in general. R1 and P1 remain open. | `RESULTS-r1-ternary-epcentre-obstruction.md`; `experiments/rule30/r1-zero-set-attack/construction/` (`direct_dfa_ternary_epcentre_synthesis.py`, `verify_ternary_epcentre_independent.py`); bounds from `RESULTS-r1-radix-automatic-period-bound.md`; imported gate implicants from `RESULTS-r1-ternary-automatic-construction.md` |
| 97 | Arbitrary-radix eventual-period test for MSB-first automatic sequences: exact iff test with safe thresholds `H=b^N`, `P=b^N lcm(1,...,N)` plus a finite addition product over `(n, n+P)` enumerating every endpoint state pair | 1 | **PROVED as a rule-independent lemma, INSTRUMENT VALIDATED; R1 remains open** | Theorem (U): for `u(n)` generated by a deterministic most-significant-digit-first automaton in radix `b >= 2` with at most `N` states, finite output alphabet and a leading-zero self-loop at its initial state (unreachable and observationally equivalent states allowed), `u` is eventually periodic if and only if `u(n) = u(n+P)` for every `n >= H`, with `H = b^N`, `P = b^N lcm(1,2,...,N)`. Extends the MSB binary lemma to any radix; the proof splits the least period `p = e d` with every prime of `e` dividing `b` and `gcd(d,b)=1`, where "`e` need not be a power of `b`; this distinction matters for composite radices", builds residual functions `R_r` on a closed tail set of `M` states, shows the equivalence chain `E_k` has `a` strict steps so `M >= a+d`, `a <= N-1`, `d <= N`, `p` divides `P`, and bounds the exceptional prefix by pumping so agreement holds from canonical length `n-M+a <= N-1` (`n <= N` the doc's count of reachable states), "the deliberately looser threshold `b^N` is safe". Finite product (C): the radix checker reads `n, n+P` in parallel high to low with carry, position in the expansion of `P`, and significant length capped at `N+1`; "Every accepted product path is a genuine pair `n,n+P` with `n>=H`, and every such pair has a path"; safe state bound `2 N^2 (L+2)(N+2)` with `L` digits in `P`; API `endpoint_pairs(delta, start=0)` returns `H`, `P`, closure count and records with `states` and `witness`, `mismatch` labels pairs by an output function. Regression: MSB product compared with an independently directed LSD addition product after reversal by output functions on all padded tables at the exhaustive entries, including unreachable states (equality per table is asserted by the script, `radix_automatic_period_bound.py:149`): radix 2 (`N=1,2,3`: 1/8/243 tables), radix 3 (`N=1,2,3`: 1/32/6,561 tables; at `N=3` 52,488 output labelings, 22,432 eventually periodic, 30,056 aperiodic), radix 4 (`N=2`, 128 tables), radix 6 (`N=2`, 2,048 tables). Worked example: base-three two-state machine `delta=((0,0,1),(1,1,0))`, outputs `(0,1)`, counting digits equal to two modulo two, exact mismatch `n=20` at `H=9, P=18`, product closes at 80 states; "The uniform theorem and the exact mismatch certify aperiodicity; the 4,096-index comparison only checks the implementation of that formula." Controls: binary regression rerun unchanged; genuine Rule 90 lone-seed neighbour passes its aperiodicity test and agrees with frozen forward evolution for 256 samples / 66,048 cells; `controls.rule90_control(6)` passes at periods 2, 4, 6; "No claim that Rule 90 has a base-three encoding is made or needed." Scope, quoted: "This is a rule-independent certificate for a specified automatic sequence. A Rule 30 construction still needs independently verified gates at every coordinate, a periodic centre, and an aperiodic observable on a zero phase. No such base-three model is claimed here." and "It does not assert that any Rule 30 diagram has an automatic presentation." Nothing about R1 is decided: "Intermediate construction machinery" that "permits a separate base-three construction search, including a period-three centre clock." "No frozen engine was edited." | `RESULTS-r1-radix-automatic-period-bound.md`; `experiments/rule30/r1-isolated-column/radix_automatic_period_bound.py`, `experiments/rule30/r1-isolated-column/radix-automatic-period-bound.json`; extends `RESULTS-r1-msb-automatic-period-bound.md` |
| 98 | Exact R1 countermodel predicate for radix-`b` MSB automatic diagrams: eventual centre periodicity and zero-set-neighbour aperiodicity decided together by one finite endpoint relation on the shared time-axis core, with the centre's period and onset left unspecified | 1 | **PROVED as a reduction (U/R); encoding specified (U/C), no run reported in this doc; R1 remains open** | Reduction proved: in the ansatz `U(t,x) = g[t mod T, x mod q, state(t,x)]`, `T=b^a`, `a>=1`, for a radix-`b` MSB core whose initial state has a paired-zero self-loop, where `T` is "a **lookup modulus**, not an assumed temporal period of column zero", with `R` core states reachable on time-axis symbols `(digit,0)`, `H=b^R`, `P=b^R lcm(1,...,R)`, `E={(Q(n),Q(n+P)): n>=H}`, output functions `C_p(Q(n))=U(Tn+p,0)`, `V_p(Q(n))=U(Tn+p,1)` and mask `M_p=(1-C_p)V_p` on the same core ("No product automaton is needed for `M`"): "The centre is eventually periodic and its zero-set neighbour is not eventually periodic if and only if both conditions hold:" (1) `C_p(s)=C_p(t)` for every phase `p` and every `(s,t)` in `E`; (2) for some phase and pair both centre labels are zero and `V_p(s) != V_p(t)`. Proof applies the arbitrary-radix period lemma of `RESULTS-r1-radix-automatic-period-bound.md` per residue subsequence to `C_p` and to `M_p`. Consequences: safe onset `T*H`, period dividing `T*P`, "neither is imposed as a guessed onset or claimed to be minimal"; ternary three-state, lookup modulus three: onset 81, period 486; binary five-state, lookup modulus four: onset 128, period 7,680. "This removes the earlier prescribed-centre-clock restriction"; "This is a deliberately broader class. Previous UNSAT results for a centre fixed from time one do not automatically exclude it." Encoding (U/C): fold the upper `a-1` low time digits through the axis table, retain the last step via `C[p,s]=g[p,0,delta(s,b*(p mod b))]`, `V[p,s]=g[p,1 mod q,delta(s,b*(p mod b)+1)]`, one-hot transition selectors, a guard per axis table, and a spanning forest of each phase's endpoint graph (at most `R-1` pairs per phase) shown to preserve the full predicate; with complete axis coverage the clauses are equivalent to (1)-(2). The canonical binary catalog supplies 21,091 reachable axes through five states; the ternary three-state backend can use all 6,561 padded labelled axis tables directly. Transfer rule: all ordinary Rule 30 gate clauses remain necessary, as does any gate implicant "that fixes actual coordinate paths and raw output labels"; the old fixed-clock variables, early centre equations, centre-violation cuts, clock-based aperiodicity selectors and clock-based raw-output pruning must be dropped; literal variable numbers may be reused only after independently verifying identical parameter allocation. Control: "Rule 90's genuine lone-seed diagram satisfies the abstract predicate: its centre is eventually zero and its neighbour is aperiodic", so the predicate must admit it with Rule 90 gates; "the automaticity lemma itself is rule-independent" and Rule 30's OR enters only through its gate constraints and any separately proved Rule-30-only prune. Verified in the doc: the reduction, "independently checked against the common-core output functions and arbitrary-radix period theorem". Not verified in the doc: "The equation-specific CNF and every imported cut still require independent reconstruction"; no SAT run is reported here. The ternary three-state instance (same `H=27`, `P=162`, onset 81, period 486, 6,561 tables, no centre clock) is run, UNSAT and DRUP-verified in `RESULTS-r1-ternary-epcentre-obstruction.md`, identified by parameter match since neither doc cites the other; no binary five-state artifact under this predicate exists on disk (every `*any-epcentre*` artifact is base-3 `N3-T3-q7`; the `R30-N5-T4-q14` artifacts are the earlier fixed-clock runs). What stays open, quoted: "Any successful candidate also needs exact all-coordinate Rule 30 gates, actual forward verification, the frozen ladder check, and the unchanged Rule 90 control before it can be reported as a counterdiagram." "No finite list of automatic classes exhausts all Rule 30 diagrams. Even a fully certified counterdiagram with an infinite-support initial row would have to be distinguished from the original lone-seed statement. No proof or kill of R1 is asserted by this reduction alone." R1 remains open. | `RESULTS-r1-automatic-full-predicate.md` (names no experiment directory); consumes `RESULTS-r1-radix-automatic-period-bound.md` and `RESULTS-r1-canonical-axis-partition.md` (catalog generator `experiments/rule30/r1-isolated-column/canonical_axis_catalog.py`); ternary instance run in `RESULTS-r1-ternary-epcentre-obstruction.md`, artifacts `experiments/rule30/r1-zero-set-attack/construction/` |
| 99 | Arm 9: factor complexity of the centre column A051023 against Cobham's linear bound for automatic sequences (Allouche-Shallit-Yassawi survey Theorem 17): pre-registered as an exact suffix-automaton `p(n)` count, run after the checkpoint correction as a fixed-`k` census `p(k,n)`, with a Thue-Morse positive control | 1 | **PROBED; route KILLED computationally; P1 unchanged** | Pre-registered measurement ("pre-registered before running"). Route logic: a `k`-automatic sequence has `p(n) = O(n)` and eventual periodicity implies automaticity for every base, so `p(n)` not `O(n)` would give non-automaticity, hence non-periodicity. Stated in advance: "This cannot prove non-periodicity from finite data"; a plateau in reach "says nothing about the limit"; "an actual proof needs `p(n)` unbounded relative to `n` for ALL `n`, which requires an inductive combinatorial argument this arm does not attempt." Checkpoint design corrected before results: the first sweep of `k` toward a fixed `n` measured only the trivial ceiling `p(k) <= n-k+1`; the corrected design fixes `k` and sweeps `n`, watching `p(k, n)` (distinct length-`k` factors in the first `n` symbols). Correctness gates, both reported as passing "before any number below is trusted": the bit-packed `_band_series("30", steps, wmax=1)` kernel cross-checked bit-for-bit against the pinned `center_column` at `n=20000`, 0 mismatches (the pinned generator at `n=5*10^6` ran 35+ minutes with zero output and was killed), and a brute-force distinct-factor count, pre-registered as a Python-set count on the first `N<=2000` bits for every `n<=N`, both sequences. The shipped script implements neither the section-1 suffix automaton nor that per-`n` gate: it counts `p(k,n)` with a sliding-window set ("not a suffix automaton", its own docstring) and gates brute-force against it at `k in {5,9,13}` at `n = --validate-n` (20000 in the recorded reproduction); no run log is on disk. Thue-Morse control: `p(k)` at its final value by `n=100000` and flat to `n=3000000` for `k` in {8,12,16,20,24}, `p(24)=76`. Rule 30 to `n=3,000,000`: `k=8` and `k=12` saturate `2^k` immediately; `k=16` reaches 65,536 by `n=1e6`; `k=20` at 988,257 of 1,048,576 (94.2%), "still climbing, no plateau"; `k=24` at 2,746,622 of 16,777,216 (16.4%), "undersampled (`n < 2^24`), uninformative". Verdict: "This is NOT evidence of non-automaticity": saturation at small-to-moderate `k` "is equally consistent with a large-period periodic (hence trivially automatic) sequence"; the discriminating regime needs `k` of several dozen and `n >> 2^k`, i.e. `n` in `10^10`-`10^13`, behind "the same `Theta(n^2)` wall as Prize Problem 3", which the arm's own speedup ("a factor-of-tens-to-hundreds gap, not the polynomial-degree gap") does not cross. The pre-registered kill condition (`p(n)/n` still growing while the control is flat, labelled "evidence against automaticity at this scale" and "the same class of statement ARM6 made") is therefore not claimed: its statistic was discarded with the first design and the verdict declines that reading of the `k=20` climb. "the factor-complexity route (ASY survey, Theorem 17/Cobham) is confirmed to have no near-term computational path to a useful answer"; "This is a negative result about the ROUTE, not about Problem 1. No row status elsewhere in this project changes." Evidence level M. Concurrent with row 76 (Morse-Hedlund `p(n) > n`, same statistic, different question): `RESULTS-subword-complexity-extended.md` 7a records that neither supersedes the other and uses ARM9's `n=10^6` values as its analyzer gate 5, `p(20)=644,259` agreeing across the two independently written pipelines. | `ARM9-factor-complexity.md`; `experiments/rule30/factor_complexity_probe.py`; ground truth `experiments/rule30/center_column.py`; fast kernel `experiments/overnight-arms/frontier_attack/a3_p2_orbit_closure/band_census.py` (`_band_series`); cf. row 76, `RESULTS-subword-complexity-extended.md` 7a |
| 100 | Fredkin second-order reversible lift `u(t+1,x) = R30(u(t))(x) XOR u(t-1,x)` (NKS notation **rule 30R**) as a reversible carrier for conservation laws, time symmetry, and reversible statistical mechanics | 1 | **KILLED BY PROOF; the route's correct form is named and is not this one** | The construction is legitimate, reversible for every rule, and was genuinely absent from this archive (repo-root grep for fredkin/toffoli/margolus/liouville returns only a "not searched" line; NKS never names 30R; OpenAlex `title.search:"second order cellular automata"` returns 15 works, none on Rule 30). It destroys the object P1 is about. **Theorem 1:** from `u(-1)=0, u(0)=delta_0`, the lift satisfies `u(t,x)=1` for every `-t <= x <= 0`, by induction carrying two rows; hence its centre column is identically `1`. Checked exactly to `t=120` for the closed cone and `t=400` for the column; a forward-60/inverse-60 reversibility control passes. The degeneracy is not Rule-30-specific hostility but it is not vacuous either: the same lift gives period 2 for rule 90, period 3 for rules 110 and 150, and no period `<= 8` in 201 symbols for rule 45, so the lift is capable of carrying an aperiodic centre and simply does not carry Rule 30's. Re-seeding as `(delta_0, delta_0)` moves Rule 30's lifted centre to `1010...`, period two. **Proposition 2:** the lifted and unlifted orbits agree only at `t=0,1` and differ at infinitely many `t`, because Rule 30's centre column is `0` infinitely often (`RESULTS-zero-tail.md` plus `RESULTS-inverse-trace.md` exclude every eventually constant lone-seed centre); measured, they coincide at exactly 2 of the first 401 rows. So any Liouville-type invariance or conserved quantity proved on the lift constrains the lift's orbit and says nothing about Rule 30's. This is the reversible-dynamics instance of obstruction E. The conserved quantities such an argument would need are independently dead here: no nontrivial local additive density through width 12 (`RESULTS-additive-conservation-probe.md`), conserved non-additive window functions KILLED to `w=12` and column-blind (`FINDINGS:83`), and any monoid-valued transfer invariant carries at most `log2` of the monoid's order in bits about the whole orbit (`FINDINGS:86`). Retained, not killed: Kari-Taati (arXiv:1311.2319v3) implement the same instinct through the **spacetime transpose** rather than a second-order lift; their Theorem 52 bijects locally conserved observables of `Phi` and `Phi^T`. Rule 30 is left-permutive only, hence not positively expansive, so their sufficient route to a transpose misses it. That check has now been run on this repo's existing space-direction column automaton `G(u,v)[t] = v[t+1] XOR (v[t] OR u[t])` and the literal candidate is **disqualified, not merely untested**: read as the pair CA `G'(u,v)=(v,G(u,v))`, its image is exactly the SFT `{(v,w) : v[t]=1 => w[t] != v[t+1]}`, of size `4*3^(h-1)` against `4^h` at visible height `h` (proved by solving the local rule, checked exhaustively for `h=1..9`), so `G'` is not surjective, entropy `log 3 < log 4`, density `(3/4)^(h-1) -> 0`. Kari-Taati define the transpose relation between surjective CA, and the time-zero slice of a CA spacetime subshift is the full shift on its alphabet, which the occurring column pairs are not. What remains open is whether the transposed spacetime subshift is conjugate to *some* CA's, which a recoding on a wider window could still achieve; the pruning experiment and its kill condition are pre-registered in section 6 of the result document. Also corrected there: Ban, Chang, Chen **and Lin** (four authors) Cor. 3.6 must not be cited as covering Rule 30 -- its hypothesis is strong transitivity, its scope qualifier is a one-coordinate potential, and the width-12 additive-conservation computation is therefore not a finite rediscovery. | `RESULTS-second-order-reversible-lift.md`; `experiments/rule30/second-order-lift/`; `experiments/rule30/column-transpose/` |
...
**C.  Single-column blindness.** A limiting functional invariant under
density-zero modification cannot distinguish arbitrary arrays that differ
on the center column. Operationally: overwrite column 0 with a periodic
word and measure its change. This is a test of the statistic alone, not a
no-go theorem for arguments using Rule-30-specific identities, quantitative
rates, or restrictions to valid seed orbits; an overwritten array generally
leaves that domain. See the scope correction in section 0.1.
...
**J.  Non-invertibility forecloses reversible structure.**  Not one of the
six above, and not itself a restatement of the Rule 90 filter (Rule 30 is
measured non-bijective at every `N` from 3 to 14, up to 3 preimages -- a
structural fact about Rule 30 itself, unlike B which is about arguments that
would also prove something false about the reversible Rule 90).  Kills row
**60** outright (no unitary evolution operator, no RMT ensemble) and row
**62** as its primary reason (no braid word: defect world lines merge and
annihilate; obstruction B only closes it as a fallback, granting a
defect-count proxy the doc itself says isn't owed).  No other row in the
register was found to die by it; not promoted to a top-level obstruction in
the paper's A-I list for that reason, but named here since it is a genuinely
distinct kill mechanism, not noise.
...
| Nersissian 2026 (Wolfram Community 3647733; Zenodo 18921456) | Exact binomial-Lucas lifting to support sets; claims `O(log n)` cell query after masked-block compression | **LOG-QUERY CLAIM REFUTED; exact support recurrence retained.**  The theorem assumes a predetermined row `m` and constant block count `k`, so it bounds only evaluation of a supplied row by `O(k log n)`.  The center has `m=n+1`; the published `compute_Sm` performs every one of the preceding `n-1` support advances and proves no sublinear construction bound.  Its own Zeta-floor section says the generational expansion cannot be bypassed.  Clean-room code reproduces the support sets and center bits, while the supplied greedy representation grows from 1 block at `m=1` to 2,993 at `m=30` (finite diagnostic only).  The P2 piling-up step also assumes false independence between overlapping block events.  See `REFUTATION-nersissian-log-query.md` and `nersissian_block_audit.py`. |
...
| Deva 2026, SSRN 5908722 | Claims `Omega(n)` decision-tree complexity from algebraic degree, algebraic immunity, and essential-variable saturation | **ADVERTISED P3 INFERENCE REFUTED.**  These quantities belong to the `t`-step center function on `2t+1` arbitrary initial cells.  P3 fixes those cells to the lone seed and supplies only the binary time index.  The all-zero Rule 30 seed retains the same arbitrary-input function but has a constant center trace, exactly refuting any degree-to-fixed-orbit inference.  Full-PDF retrieval was blocked, so this audit is limited to the abstract's advertised implication.  See `CLAIM-AUDIT-deva-dual-flow.md`. |
...
## 11. Correction, 2026-09-09: R7's residual is not "the same zero-set question as R1"

Section 4's R7 entry ends (line 461-463) with "an escape at `q = 0 mod 420`
cannot be time-periodic at all, so it must be exactly a7's aperiodic object,
i.e. the same zero-set question as R1."  The first clause stands.  The last
clause is wrong as a statement, and is corrected in
`RESULTS-four-convergent-statements.md` §2.3.

R1's statement quantifies over the lone-seed diagram.  The ladder's mode-(ii)
question quantifies over `L(R,k)`, a sound **over-approximation** containing
letter words with no realizing configuration (7.3.F).  Only one direction
transfers: mode (ii) EMPTY implies `Thm(2)` implies R1's `p=2` instance.  A
ladder *witness* implies nothing about R1 — and the relaxed form of R1's
conclusion is already **false at `q=1`**, by the word extracted at
`R=1, k=2, w=(0,1)` in `experiments/rule30/r1-zero-set-attack/RESULTS-R1-ZERO-SET-ATTACK.md`
§3.  Treating the two as one question invites exactly the inference that does
not hold.

Two further corrections from the same audit:

* **R1 is equivalent to Problem 1, not a subgoal of it.**  Section 4 records
  that closing R1 closes P1 (line 240).  The converse also holds: P1 falsifies
  R1's antecedent, so R1 is vacuously true if P1 is.  R1 is a change of
  coordinates that localizes the residual freedom to `rho`; it is not a weaker
  target.
  *Dependency, read before building on this:* the forward direction uses the
  **eventual** form of the width-two theorem — no nonzero finite seed has two
  adjacent eventually periodic columns — attributed here to Kopra TCS 946
  (2023) 113668 Thm 3.5.  Jen 1990 Prop. 3's non-eventual form would yield
  only "`c` is not periodic".  That citation was **not** re-verified against
  the paper; see `RESULTS-four-convergent-statements.md` §5.  The `p=1` case
  does not depend on it: rows 25 and 26 exclude every eventually constant
  lone-seed centre outright.
* **a7's i.o. lemma is proved**, as of
  `RESULTS-ladder-rung3-io-and-aperiodicity-audit.md` §1, and is logically
  independent of the periodicity question.  It should not be carried as an
  open convergent statement anywhere.

**Primary-source follow-up, 2026-09-09:** the Kopra dependency above has now
been checked directly. Journal Theorem 3.5 and Corollary 3.7 exclude
eventual width-two periodicity for every nonzero left-finite configuration,
even with an arbitrary right tail. The S1/P1 equivalence stands. See
[RESULTS-kopra-eventual-verification.md](RESULTS-kopra-eventual-verification.md).

**Logical-seam follow-up, 2026-09-09:** §11's assertion that the first
clause of the quoted R7 sentence stands is withdrawn. As already explained
in the rung-3 audit §6, a bounded torus census does not prove that every
q=420 escape must be aperiodic. Also, the q=1 witness refutes a particular
relaxation, not every possible q=420-multiple certificate. The converse
from S1(2) to such a certificate remains unproved. See
[RESULTS-load-bearing-seam-audit.md](RESULTS-load-bearing-seam-audit.md) §4
and the four-statement report's appended §8 for the polarity and universe
corrections.

**Overnight Tasks 3–6, 2026-09-09:** the width-24 gap-set graph is now
closed and independently verified. Uniform family exclusions are proved,
but the combined strip/cone model still admits `(2,4^m)` for all m>=3.
The direct zero-cost strip abstractions cannot prove mortality, and C3's
source-state closure remains open. Two false-extinction paths in the gap
explorer are repaired. See
[RESULTS-overnight-tasks-3-6.md](RESULTS-overnight-tasks-3-6.md) for the
verified findings, explicit coverage, and links to all four reports.

**Variable-length episode follow-up, 2026-09-09:** an exact composition law
now carries the accumulated suffix across episode switches, with every
intermediate two-bit matching condition retained. Resetting that suffix
has an explicit false-continuation witness. At any onset of active length
r, a scalar-zero episode lasts at most `r-1` updates and a scalar-one
episode at most `r`, by a uniform reconstruction proof. These results do
not establish mortality or close P1/P2. See
[RESULTS-variable-length-episode-composition.md](RESULTS-variable-length-episode-composition.md)
for the law, proof, and independent finite verification.

**Episode continuation, 2026-09-09:** a preceding update's terminal pattern
sharpens the next zero-run bound to R-3 at length R>=3; scalar-one runs
have the stronger arbitrary-frontier bound R-2 for R>=2. These credits do
not accumulate with switches. Exact inverse fibers and accepted-history
mergers rule out a simple information-loss charge. A total repeat budget
survives the complete length-12 census but is unproved. Exact prescribed-tape
languages have been executed; their minimized state counts grow rapidly.
See [RESULTS-episode-memory-and-repeat-budget.md](RESULTS-episode-memory-and-repeat-budget.md).

**Repeat-budget continuation, 2026-09-09:** a nonlinear first-defect rank
decreases at every repeated scalar, but its defining height resets exactly
at switches. Same-phase returns increase the rank by elapsed time, and
switch refill is unbounded. Phase-aware additive scan ranks are exactly
infeasible on second images in each sector; pumped third-image histories
exclude ranks from inverse crossing counts plus phase, sector and length.
The cumulative repeat budget and mortality remain open. See
[RESULTS-repeat-budget-phase-reset.md](RESULTS-repeat-budget-phase-reset.md).

**Consolidated episode record, 2026-09-10:** the episode program now has a
single index of its proved composition law, sharpened constant-run bounds,
phase-height reset/refill identity, exact inverse fibers, pumped rank
obstructions, additive-rank certificates, regular prescribed-tape languages,
and the remaining cumulative-refill theorem. See
[RESULTS-episode-program-synthesis.md](RESULTS-episode-program-synthesis.md).
