...
Across twelve LLM-driven code-evolution systems whose fitness specification was
read directly, only one documents an instrument-validation failure, and it is
a failure of the *correctness* sandbox, not the timer itself. Extended
2026-09-05 from six to twelve systems; see the addendum below for what changed
and what is newly confirmed.
...
| SATLUTION (arXiv:2509.07367) | PAR-2 (penalized avg runtime, 5000s timeout) on SAT Competition instances | yes, PAR-2 is a runtime score from distributed benchmark infra | none found; only solver correctness (Stage 1-2 verification) is validated, not the timing infra |
| AdaEvolve (arXiv:2602.20133) | inherits the wrapped task's `evaluate()` across 185 problems; its own contribution is scheduling, not a new fitness/timer | not specified for the family as a whole in the material accessed | not found in the material accessed |
| CodeEvolve (arXiv:2510.14150, EMNLP 2026 Findings) | problem-dependent `f_sol` on the AlphaEvolve benchmark suite | timing is a sandbox *constraint* (60s/360s limits), never an optimization objective | none found beyond sandbox-correctness checks |
| EoH (arXiv:2401.02051, ICML 2024 Oral) | combinatorial-optimization objective value (e.g. bin-packing excess) on 3 benchmarks | no | not found in the material accessed |
| ReEvo (arXiv:2402.01145, NeurIPS 2024) | COP objective value across 6 combinatorial problems | not established from what was read | not found (abstract-level access only) |
| Sakana AI "AI CUDA Engineer" (pub.sakana.ai/ai-cuda-engineer, Feb 2025; fix in arXiv:2509.14279) | measured wall-clock speedup of generated CUDA kernels vs.\ PyTorch (KernelBench) | yes — timing *is* the entire fitness signal | **documented failure, not absence**: external testers found the eval sandbox had a memory-reuse exploit letting kernels skip correctness checks, inflating claimed speedups (avg 3.13x, peaks 10-100x) down to a re-measured ~1.49x; confirmed by the same author team's follow-up introducing "robust-kbench" to close the loopholes |

**Bounded negative.** This covers twelve systems read directly (six 2026-08-31,
six more 2026-09-05). It is *not* a code-search sweep: `api.github.com/search/code`
still returns 401 unauthenticated, arXiv search is metadata-only (title/abstract),
and AdaEvolve/ReEvo were read at abstract level only (paywalled or
fetch-summarized), so their "not found" verdicts mean exactly that, not a
confirmed absence. No search-based completeness claim is available. State the
bound in the paper.

**Scope this claim to LLM-driven code evolution specifically.** Pre-LLM
ML-compiler autotuning (AutoTVM/Ansor) continuously validates its *learned
cost model* against real hardware latency inside the search loop. That
validates a model, not the raw timer, and real measurement is still trusted as
ground truth there — so it doesn't contradict the gap above, but "nobody
validates their instrument" must not be stated as a claim broader than the
LLM-code-evolution family.

**The Sakana AI CUDA Engineer row is the most valuable addition.** It is the
one documented case in the whole family of a fitness/measurement instrument
actually failing unvalidated, caught only post-release by outside testers,
with the authors' own admission and fix as the primary source — not press
coverage relaying it. It sharpens the paper's motivating point (measurement
bias is a real, recurring risk in this family) without weakening the
prior-art decision below, since the failure mode there is a correctness
exploit, not the granularity question this instrument addresses.

### Citation corrections, 2026-09-05

Verified against primary sources fetched fresh (not against this file's own
prior claims, which were the thing under review):

* **Binder et al. is wrong; it is Binder \& Hulaas** (two authors). Cite as
  such; do not write "et al." for a two-author paper.
* **Cachegrind/Callgrind needs a real bibitem, not a bare tool+year.** Anchor
  to Nethercote \& Seward, "Valgrind: A Framework for Heavyweight Dynamic
  Binary Instrumentation," PLDI 2007, pp. 89-100, plus a dated manual-URL
  footnote for the quotes. The phrase this file originally quoted ("does not
  measure time spent... independent of current system load") traces to the
  **Callgrind** manual, not Cachegrind's, and could not be reconfirmed
  verbatim on the current live `cl-manual.html` (it may have been edited
  since, or was itself a paraphrase). What *is* independently confirmed today
  on `cg-manual.html`: "Cachegrind measures the exact number of instructions
  executed by your program, not an approximation" and "instruction counts are
  highly reproducible; for some programs they are perfectly reproducible."
  The substance survives; the specific sub-quote does not.
* **EIP-2565's word-granular formula is not inherited from EIP-198.** EIP-198
  (2016) charges a piecewise function of raw *byte* length; EIP-2565 (2020) is
  the one that introduces `ceil(max_length/8)`-granular, word-level charging.
  This makes EIP-2565 a cleaner, more specific antecedent than originally
  framed — strengthens the citation, does not weaken it. Its formula
  (`words = ceil(max_length/8); multiplication_complexity = words**2`) and
  Final status were both confirmed verbatim against the live EIP text, as was
  its successor EIP-7883's Final status.
* **Bouras, Hanna \& Petke: verbatim abstract confirmed** ("perf's cycles
  measure yields the best overall performance, outperforming Time by 5.1%"),
  and the 5.1% is an aggregate across MAGPIE's 7 benchmarks / 13 fitness
  functions, not a single cherry-picked case — supports the prior-art claim
  as strongly as originally stated. Cite the LNCS volume's 2026 publication
  year (SSBSE 2025 the conference, Seoul, Nov. 2025; "First Online" 2026-04-30
  in the Springer record), not 2025.
* **Two DOIs left unconfirmed rather than guessed:** Jin \& Branke (IEEE
  Xplore access blocked; vol/issue/pages/year otherwise match) and Stabilizer
  (two conflicting ACM DOI records for the same paper; page range not
