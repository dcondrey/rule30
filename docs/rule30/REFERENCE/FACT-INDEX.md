...
Updated: 2026-09-02 (STALE: 27 new experiments uncommitted as of 2026-09-11)
...
`EXPERIMENT-ATLAS.md` (updated 2026-09-11).
...
```text
M_c(n) <= n+1 for every n>=7 and c in {2,3}
```
...
for `7<=n<=29`, exhaustively over all `2^n` sources at each length (small DLP
cases need their own guards).  The bound is **not** asymptotically tight:
`g(n)=max_c M_c(n)` fits `0.871 n + 0.24` over `7<=n<=29`, so the slack
`g(n)-n` declines at `-0.129` per unit `n`.  The decline is not monotone and
the top three lengths run against it, `-5, -3, -2` at `n=27,28,29`; each added
length has raised the slope, from `0.842` at `n<=27`, so the margin under the
`0.95` sufficient slope is `0.079` and shrinking.  The `n=15`
saturation is therefore a small-`n` artifact, not a near miss, and a proof needs
only a lossy asymptotic bound (`g(n) <= 0.95 n` would do) plus the finite census
below `N_0`, which is now supplied free to `N_0 <= 30`.  A related core audit finds 88/90 DLP formulas through
...
| Run reach, sufficient direction only (replaces the retracted one-column reach lemma; the "exactly when" form is also retracted) | A run of ones forces: `c(t)..c(t+k-1)` all 1 implies `col_{-k}(t) = c(t+k)` for even `k`, `NOT c(t+k)` for odd `k`. **The converse is false** — 116 of the 256 length-8 centre words force deeper than their run prefix, and `01010` forces `col_{-4}=0` with depths 1-3 free, via the relation `col_1(t) AND col_1(t+2) = 0` between unknown cells. Rule 90 leaves that cell free, so the screen passes | `NEXT-DIRECTIONS-post-rung2.md` 1; `experiments/rule30/reach/` |
| Support-count exclusion for a periodic centre (second resource parameter) | Counting left-row **ones** instead of left depth. `H(K)` = deepest left prefix reconstructible on `<= K` ones; finite `H(K)` excludes every configuration with `<= K` left ones, at any spacing. Alternating centre: `H_01(26)=191`, `H_10(26)=119`, giving `(01)^96 0 => >=27` ones in the first 192 left cells and `(10)^60 1 => >=27` in the first 120. Strictly stronger than the depth family (`d<=24`), which cannot reach sparse far-left rows. Those tables are under the **no-`11`** model (necessary for an alternating centre only). **Runs for every nonconstant word of period 2-5, all 52 phases, all terminating** under the weaker general one-step model `u(t)=1 => u(t+1)=1-c(t)`; depth-per-one 1.8 to 8.4, minimum not collapsing with period, all-one word exactly 2 (the proved checkerboard case). **The two models are not interchangeable** and their `H` values differ (`01` gives 1,7,17,23,33 weak against 1,7,17,21,28 no-`11`); mixing them put a wrong table in this file once | `RESULTS-alt-trace-fiber.md` |
| Generalised two-branch cost identity | On the frontier scan, flipping the fed value flips every diagonal entry, and the induced change in the next initial-row output is `delta = ((T+1) mod 2) XOR c_T XOR parity(A_1..A_T)`; branch costs `kappa_0 = q_0`, `kappa_1 = 1 + (q_0 XOR delta)`. 0 violations over 2,520 reachable states, seven words of period 2-5. The centre word enters through the single bit `c_T`; omitting it (valid only where the phase pins `c_T=0`) fails on 2,560 of those states | `RESULTS-alt-trace-fiber.md` |
| Zero-cost mortality: the one remaining obligation for the periodic subcase | Every state has at most one zero-cost successor, so *every finite support budget is eventually exceeded* iff *no frontier reachable from a finite legal prefix has an infinite legal zero-cost orbit*. Both the depth and support certificate families reduce to this. The 7x4 torus (rows `0000001/1000011/0100110/1111101`, cyclic) is a genuine infinite spacetime with alternating centre and left ones at depths `1,8,15,...`, so `H(K) >= 7K` over actual-right configurations and **no support density above 1/7 is provable**; measured `H_01(K) = 7K` exactly at `K=1,3,4,5,7,8,9` | `RESULTS-alt-trace-fiber.md` |
| Exact centre-only reach on the lone seed | Forcing at depth `k` depends only on `c(0..k)` and is decided exhaustively by the `2^k` initial right rows. Maxima `15, 27, 27, 31, 31` over horizons `t<100, 500, 1000, 2000, 4096`, depths tested to 63; attained at run length 0 or 1. Certificates verified over all right rows: `t=61` k=15, `t=243` k=23, `t=170`/`t=171` k=27, `t=1046` k=31 (value 1), `t=1987` k=31 (value 0). **No asymptotic law is claimed**; obstruction A's `O(log t)` reach claim is unproved, not restored. A one-step right-column relaxation gives only lower bounds (9/13/16/18/18) and recovers none of the six certificates | `NEXT-DIRECTIONS-post-rung2.md` 1 |
| Left-frame recurrence | `s(t,-t+j) = L_{j-2}[t-1] XOR (L_{j-1}[t-1] OR L_j[t-1])`, exact; the OR is self-referential and absorbing, unlike the right frame's `E_j` where it feeds a running parity | `RESULTS-ordered-wedge-glide.md` 2 |
| Glide form of NKS p. 871 (MEASURED) | Left-frame periodicity of period `g` to depth `D` is exactly `s(t+g,x-g)=s(t,x)` on `-t <= x <= -t+D`. Depth table `3, 8, 29, 400, 87867` for periods `2,4,8,16,32`, reproduced to the unit; the period-16 plateau **ends** at `j=87,867`. Rule 90 control: left-frame period exactly `j`, no ordered region | `RESULTS-ordered-wedge-glide.md` 0--2 |
| Ternary three-state automatic countermodel class excluded (C certificate, K on the class) | No padded three-state, base-three paired-digit MSB automaton with output lookup periods 3 (time) and 7 (space) describes a Rule 30 right half-diagram (`t>=0, x>=1`, infinite initial support allowed) whose centre is eventually periodic and whose right neighbour is aperiodic on the centre-zero set; centre period and onset unrestricted (`H=27, P=162` from the radix period theorem; an eventually periodic centre then agrees at shift 486 from time 81, a consequence of the automaton). All 6,561 padded axis tables covered, no finite witness horizon; UNSAT with independently rebuilt CNF and DRUP VERIFIED. **Retires this class only**: does not bound automaton size of general Rule 30 diagrams, exclude nonautomatic diagrams, or decide R1 | `RESULTS-r1-ternary-epcentre-obstruction.md`; `experiments/rule30/r1-zero-set-attack/construction/` |
| Arbitrary-radix automatic eventual-period bound | For `u(n)` from a deterministic MSB-first radix-`b` automaton (`b >= 2`) with at most `N` states, finite output alphabet and leading-zero self-loop (unreachable and observationally equivalent states allowed): `u` is eventually periodic iff `u(n)=u(n+P)` for every `n>=H`, with `H=b^N`, `P=b^N lcm(1,...,N)`; agreement already holds from canonical length `<= N-1`, and `b^N` is a deliberately loose safe threshold. Rule-independent. The finite MSB addition product over `(n, n+P)` enumerates every endpoint state pair (state bound `2N^2(L+2)(N+2)`, `L` = digits of `P`); compared with an independently directed LSD product over all padded tables, exhaustive at radix 2 (`N<=3`), radix 3 (`N<=3`), radix 4 and 6 (`N=2`). Certifies nothing about Rule 30: a construction still needs independently verified gates at every coordinate, a periodic centre, and an aperiodic observable on a zero phase, and no Rule 30 diagram is asserted to have an automatic presentation | `RESULTS-r1-radix-automatic-period-bound.md`; `experiments/rule30/r1-isolated-column/radix_automatic_period_bound.py` |
| Automatic-diagram R1 countermodel predicate | For a radix-`b` MSB core with a paired-zero self-loop at its initial state defining `U(t,x)=g[t mod T, x mod q, state(t,x)]`, `T=b^a`, `a>=1`, `T` a lookup modulus (not an assumed period), `R` core states reachable on time-axis symbols, `H=b^R`, `P=b^R lcm(1,...,R)`, `E={(Q(n),Q(n+P)): n>=H}`, `C_p(Q(n))=U(Tn+p,0)`, `V_p(Q(n))=U(Tn+p,1)`: the centre is eventually periodic AND its zero-set neighbour is not eventually periodic iff (1) `C_p(s)=C_p(t)` on every pair of `E` for every phase `p<T` and (2) some phase and pair have both centre labels 0 and `V_p(s)!=V_p(t)`. Safe onset `T*H`, period divides `T*P`, neither imposed nor claimed minimal. Rule-independent: Rule 90's lone seed satisfies it, so Rule 30's OR enters only through gate constraints and any separately proved Rule-30-only prune. Exhausts no non-automatic diagram; an infinite-support counterdiagram would still have to be distinguished from the lone-seed statement; asserts no proof or kill of R1 by itself | `RESULTS-r1-automatic-full-predicate.md`; period lemma from `RESULTS-r1-radix-automatic-period-bound.md` |
...
| Bounded-degree `F2` certificates over raw seed bits, Nullstellensatz or polynomial calculus, for the finite alternation systems | Polynomial-calculus degree equals the axiom floor at every placement and generator set through `w=14`, with per-width maximum `n = w-2` at every `w=4..14`; two independent implementations agree on all 117 placements `w<=13` | `experiments/rule30/p1-period2-invariant/RESULTS-PT2-PC-DEGREE.md`, `pt2_pc_degree.py`, `pt2_pc_degree_crosscheck.py` |
...
|---|---|---|
| A | `O(log t)` propagation cannot meet a `Theta(t)` target. Right boundary only: the left frame reaches depth `3, 8, 29, 400, 87867, >2.1e9` with period `2..64`, super-exponential in `log P` rather than linear. Both still need unbounded period at depth `Theta(t)` | `PATH.md` 7.3A; corrected by `RESULTS-ordered-wedge-glide.md` 3 |
