...
A bounded Crosstalk run tested this handoff directly.  Six proposals were
rejected: three retained linear frontier work, and three asserted an affine or
polynomial transfer monoid without defining the nonlinear seam map.  The only
proposal with complete executable pseudocode used a truncated
`GF(2)[x]/(x^k)` correction state.  Reproducing that code exactly gives its
first counterexample at `n=5` (`candidate=0`, `oracle=1`).  See
`transfer_monoid_probe.py` and `RESULTS-crosstalk-arm8.md`.
...
uv run python transfer_monoid_probe.py --max-index 4096
```
