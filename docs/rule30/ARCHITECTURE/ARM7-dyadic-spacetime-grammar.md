...
A deterministic greedy discriminator selected 10, 20, 17, 12, and 8 perimeter
positions at scales 4, 8, 16, 32, and 64 respectively at horizon 512.  Across
horizons 64--1024 the selected counts at fixed scales continued to grow (for
scale 16: 5, 8, 13, 17, 21).  Random tiles require comparable counts after
accounting for their vocabulary, and the chosen coordinates change with the
sample.  This is therefore not yet a Rule-30-specific boundary algebra.

Flipping zero and one only inside the active causal triangle also produces no
new simplification.  At horizons 64--512 the complemented Rule 30 DAG exponent
is 1.363 versus 1.367 for the original; the horizon-512 DAGs contain 3,710 and
3,704 nodes.  Their tile vocabularies agree exactly from scale 16 upward.  The
hierarchical reuse belongs to the complete binary relation rather than a
separate grammar of marks or voids.
...
uv run python boundary_signature_probe.py \
  --horizons 64 128 256 512 1024 --scales 4 8 16 32
```
