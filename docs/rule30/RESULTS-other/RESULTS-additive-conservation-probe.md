...
**Prior-art check, 2026-09-17.  Three findings, all resolved: Ban et al. never
reached Rule 30, the additive line was already cleared elsewhere but uncited
here, and Formenti-Kari-Taati is a structural result that does not collide but
sharpens what this document's negative may claim.**

*Ban, Chang, Chen and Lin: settled by the primary, and it never reached Rule 30.*
The NCTS preprint was obtained on 2026-09-17 and is archived at
`references/ban-chang-chen-lin-2009-permutive-preprint.pdf`.  Its working
definition of "permutive" requires a one-sided rule to be one-sided in its
**dependence** as well: leftmost permutive *and* independent of `x_i` for
`i > 0`, or the mirror, or bipermutive.  Rule 30 reads `x_1` through the OR, so
it fails every case, and the paper never mentions Rule 30.  The hypothesis class
excludes it outright, so this document's sweep is independent of that paper
whatever its published numbering.  **Do not cite Ban et al. for Rule 30.**  Full
derivation in
[RESULTS-second-order-reversible-lift.md](RESULTS-second-order-reversible-lift.md)
section 5.

*The additive line itself is already cleared, but not from this document.*  The
prior art for site-sum and additive conserved quantities, Hattori and Takesue
(Physica D 49, 1991) with its decision procedure, Boccara and Fuks (J. Phys. A
31, 1998), and Pivato (Nonlinearity 15, 2002), is catalogued in
`experiments/overnight-arms/frontier_attack/a14_row53_conserved/row53_conserved_windows.md`
under "What is already catalogued, so that nothing below is presented as
rediscovery".  That clearance was never linked from here, which is why this
document reads as an uncited sweep.  It is cited now.

*Formenti-Kari-Taati checked, 2026-09-17: does not collide, but sharpens the
claim this document may make.*  Obtained and read in full
(`references/formenti-kari-taati-2011-hierarchy-of-conservation-laws.pdf`).  Its
published decision procedure (Theorem 2) is correct and terminating for any
finite window in principle, but every worked example stops at width 1, no rule
number (30, 90, 150, 184, ...) appears anywhere in the paper, and its one
shortcut theorem (Corollary 3: positively expansive CA have no non-trivial
real-valued conservation law) does not fire for Rule 30, which is left-permutive
only.  So this document's width-12 negative is independent.  **One correction
this paper forces:** its own Example 2 (Rule 150, positively expansive) has no
non-trivial real-valued law but does have a non-trivial `Z_2`-valued one on the
same window.  This document's result must therefore be read as "no non-trivial
*additive/real-valued* local conserved density through width 12," not "no
conserved quantity of any kind" -- the group-valued case for Rule 30 is open and
untouched here.  Full derivation in
[RESULTS-second-order-reversible-lift.md](RESULTS-second-order-reversible-lift.md)
section 5.
