...
* **Cross-width comparison is meaningful but does not supply uniformity.**
  The functions satisfy `f_(m+1)(0,b)=f_m(b)`, so their circuit complexities
  form a meaningful sequence. Independent bounded synthesis supplies no
  uniform, costed construction or evaluation for all widths. See the
  [scope correction](AUDIT-automaticity-route-scope.md#5-row-93-has-a-separate-scope-correction).
