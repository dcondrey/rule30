...
### 6.1 Constraints on route 1 recorded after this review (2026-09-19)

Route 1 has not been attempted under its own name, but two later results
bound it from both sides.

- **Full seam, measured.**  The Hashlife row-block recursion is an exact
  identity (`RESULTS-other/RESULTS-hashlife-center.md`, record
  `e31307608a192f0a`), so it is the literal "full seam" carrier.  Its
  memoized node count on the lone seed is superlinear at every measured
  depth: 501,531 distinct `advance` calls at `n = 8192`, about `61.2 n`,
  fit `n^1.57` (record `98afc5e2b6a751a5`; Rule 90 control `n^0.14`).
  Finite evidence only.
- **Bounded seam, reduced to automaticity.**  One finite stock serving
  every level would make the centre column 2-automatic
  (`RESULTS-p3/RESULTS-p3-two-channel-window-decoder-refuted.md` section 4,
  record `r30-p3-two-channel-bounded-renormalization-automaticity`).  ARM6
  separates 8,191 kernel residuals through depth 12, so such a stock needs
  thousands of elements; non-automaticity itself is not proved.  The
  DRAT-refuted automatic classes of the R1 construction track are P1
  period-two objects and do not bear on this.

What survives is a level-growing quotient of Hashlife nodes strictly
coarser than node identity.  For P2 the readout is the centre-segment
summary `(n, D, A, I)` composed by the concatenation law of section 2.1,
which is coarser than the P3 centre-bit readout: `+--+` and `-++-` both
have summary `(4, 0, 0, 2)` and are interchangeable in every parent
summary.  So route 1 is not route 4 in disguise.  Whether a congruence on the
reachable lone-seed node DAG that refines the summary compresses that DAG
is unmeasured.  Kill for the exact form: no such congruence found has a
class count below a constant fraction of the raw node count over the
measured depths.

Surviving that kill would still not be P2 progress.  An exact compressed
seam only computes `(D, A, I)` faster; it bounds nothing, which is
condition 4 of section 3.  The composition law itself carries no
cancellation: it is an identity on arbitrary words, so any bound derived
from it alone also holds for the all-ones word.  Writing a level-`k+1`
piece as `uv`,

```text
I(uv) - I_u - I_v = n_v D_u^2 + 2 D_u A_v,
```

and the halves of shell `k+1`, `[2^(k+1), 3*2^k)` and `[3*2^k, 2^(k+2))`,
are not shells, so bounding the right side from the halves' own bounds is
the triangle inequality over aligned blocks: at depth `j` it needs
`sum |D|` over the `2^(K-j)` aligned blocks of shell `K` to be `o(2^K)`,
which is stronger than P2 because the absolute value sits inside the sum.
The only place Rule 30 can enter route 1 is an estimate of the cross
term, i.e. of how `D_u` and `A_v` of adjacent aligned blocks are coupled
through the spacetime between them.  That is route 2's residual or route
3's boundary term, so route 1 is a carrier for routes 2 and 3, not an
independent route.

The local forms of routes 2 and 3 were then tested
(`experiments/rule30/p2-seam-routes/RESULTS.md`): Rule 30 has no spatially
twisted local additive conservation law of window `<= 11` for any periodic
weight of period 1, 2, 3, 4, 6 or 8, so a route-3 interior current cannot be
removed locally; and the lag-`2^j` residual after the Rule 150 linearization
depends on every cell of the window except `x_(-L)` and is balanced on the
orbit, so route 2's residual region is the whole cone. Both routes survive
only with growing windows, history, or identities valid on the orbit rows
alone. The last of those three is itself bounded below by record
`r30-p2-orbit-window-horizon-refuted-or-vacuous`
(`experiments/rule30/p2-orbit-window-horizon/RESULTS.md`): on the first `2^20`
lone-seed rows a fixed-radius orbit-only boundary correction
`K_t - rho = h(W_r(t+1)) - lambda h(W_r(t))` is refuted for every `h`,
`lambda` and `rho` at every `r <= 18`; at `r = 19` only the centred case
`lambda = 1, rho = 1/2` falls; and at `r >= 20` no window repeats, so nothing
there is tested rather than allowed. The orbit-only branch therefore gains no
support from these rows at any fixed radius they can test, while growing
windows, history and sums that are not a fixed-radius local function remain
open.
