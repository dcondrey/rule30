...
single checkerboard, the same for every right half.  Every nonzero member of
either fiber carries infinitely many ones, so the union of the two fibers
contains exactly one finitely supported configuration, the zero row.  For
support radius $w$, the sharp maximum
constant-prefix length is $2\lceil w/2\rceil+1$ when the initial center is zero
and $2\lfloor w/2\rfloor+2$ when it is one.  The maximum over both constants is
therefore $w+2$, attained by exactly $2^w$ configurations for even $w$ and
$2^w-1$ for odd $w$.  Hence no column of a nonzero finite Rule 30 orbit is
eventually constant.
\end{abstract}
...
Wolfram's Prize Problem~1 asks whether the center trace from the lone-seed
configuration is eventually periodic \cite{wolfram}; Kopra records the broader
...
\cite[Thm.~2b]{jen1986}\cite[Prop.~3]{jen}, while Kopra's width-two theorem
does not decide whether that single exceptional column exists
\cite{kopra}.  We settle the entire eventual-period-one subcase for every
nonzero finite configuration, a class strictly broader than the lone seed.
This is a direct partial result toward the center-column question, but it does
not resolve Prize Problem~1 and leaves every nonconstant eventual period open.
...
Two prior results bound what is new here.  Jen gives necessary and sufficient
conditions for a finite initial condition to generate an eventually-one
temporal sequence \cite[Thm.~7a]{jen1986}.  Rule 30 meets that theorem's
hypothesis $a_0=0$, $a_1=a_4=1$ but none of its four alternatives: their
coefficient conditions require, respectively, $a_6=a_7=1$, $a_6=1$,
$a_7=1$, or $a_5=1$, whereas Rule 30 has $a_5=a_6=a_7=0$.  The necessity
direction therefore already excludes an eventually-one column for every
finite Rule 30 configuration.  We do not claim novelty for that finite-support
exclusion.  The results here instead give both exact fixed-locus fibers,
establish the Rule-30 zero exclusion explicitly, and supply sharp quantitative
horizons and extremizer counts.  Jen remarks that symmetric results hold for
constant-zero sequences, but does not state a Rule-30 zero-fiber classification
or any of these quantitative conclusions.  Separately, inversion for
nearest-neighbor rules has been solved for the \emph{additive} class over
$\mathbb Z_p$, by an operator
...
from the origin.  The next unresolved case is eventual period two; no claim
about that case or any higher period is made here.

That case is genuinely harder, and in a way the present method makes precise.
Write $H(p,w)$ for the sharp maximum length of an eventually $p$-periodic
central prefix over rows of support radius $w$.  Theorems~\ref{thm:horizon}
and~\ref{thm:onehorizon} together compute $H(1,w)=w+2$ exactly.  For $p=2$ no exact law is known, and no bounded
one can exist: left permutivity supplies, for any prescribed trace of length
$N$ and any compatible right prefix, a unique left half and hence a finite row
of support radius $\approx N$ realizing that trace through time $N$, so
$H(2,w)\ge w$ for every $w$.  A constructive check of this bound, evolving each
derived row with the ordinary Rule 30 map, gives $H(2,w)-w\in\{1,2,3,5\}$ for
$w\le 300$, and reproduces $H(2,1)=6$.  Consequently $H(2,\cdot)$ is unbounded
and the constant case is not merely the first of a uniform family: the
$p=1$ horizon is sharp and finite in $w$, while the $p=2$ horizon grows at
least linearly.  Exhaustive search confirms the difficulty from the other side,
returning UNSAT for every instance of a period-two exclusion grid through
$n=30$; a general structural account of that exclusion, beyond the small-radius
regularities we have observed, remains open.  We make no conjecture
here about whether $H(2,w)-w$ is bounded.

\section{Reproducibility and formalization}\label{sec:repro}

The accompanying source archive was regenerated against repository snapshot

\begin{center}
\small\texttt{99b2d82af113e385f43452eb2b8f1b22180e6de0}.
\end{center}

From the repository root, the following commands reproduce the computational
checks and figures.

\begin{verbatim}
cd docs/rule30/paper
uv run python ../../../experiments/rule30/constant_trace_audit.py
uv run python ../../../experiments/rule30/figures.py --check --out figures
...
\pagebreak
The exhaustive audit must end with \texttt{FAILURES: none}.  For
$w=1,\ldots,7$, its zero-horizon/count pairs are
\texttt{2/1, 2/3, 4/7, 4/15, 6/31, 6/63, 8/127}; its one-horizon/count pairs
are \texttt{1/2, 3/4, 3/8, 5/16, 5/32, 7/64, 7/128}.  These are horizon
indices, so the associated prefix lengths are one larger.  The figure command
must print \texttt{figures: all claimed properties hold}; it generates the
three panels of Figure~\ref{fig:panels} and the six panels of
Figure~\ref{fig:horizon}.  The SMT report must return status \texttt{PASS}
with all 18 local Boolean obligations \texttt{unsat}.  The paper has
no numerical tables.  A successful \LaTeX{} build produces a six-page PDF.

The Lean~4 file \texttt{experiments/rule30/Rule30ZeroTail.lean} is explicitly
partial.  It formalizes the local Rule 30 equation and left permutivity,
arbitrary-depth triangular sensitivity, the prefix-OR formula conditional on a
first right-hand one, the Boolean identities used for
$F(\mathcal C_m)\subseteq\mathcal C_{m-1}$ and
$F(\mathcal C_1)\subseteq\mathcal C_1$, and the infinite-left-tail
contradiction conditional on the classification.  It does not formalize the
derivation of that classification from a zero trace, the all-one fiber, either
sharp horizon theorem, or the eventual-constant corollaries.  With Lean
4.33.1 installed, it is checked by

\begin{verbatim}
lean ../../../experiments/rule30/Rule30ZeroTail.lean
\end{verbatim}

which should exit successfully without output.  Lean was not installed in the
environment used for the present editorial rebuild, so that command was not
rerun here.  Every cell in both figures was regenerated directly from
\eqref{eq:rule30}; the exhaustive and SMT checks supplement, but do not replace,
the unbounded proofs above.
